...
@ut.skipUnless(
    h5py.version.hdf5_version_tuple < (1, 9, 178),
...
class TestSwmrNotAvailable(TestCase):
    ...
    def setUp(self):
        ...
        self.dset = self.f.create_dataset(
            "data", chunks=(13,), maxshape=(None,), data=self.data
        )
...
@ut.skipUnless(
    h5py.version.hdf5_version_tuple >= (1, 9, 178), "SWMR requires HDF5 >= 1.9.178"
)
...
class TestDatasetSwmrWrite(TestCase):
    ...
    def setUp(self):
        ...
        self.dset = self.f.create_dataset(
            "data", shape=(0,), dtype=self.data.dtype, chunks=(2,), maxshape=(None,)
        )
