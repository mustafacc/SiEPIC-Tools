...
@ut.skipUnless(
    h5py.version.hdf5_version_tuple >= (1, 8, 11),
...
@ut.skipUnless(
    ...
    "Direct Chunk Reading requires HDF5 >= 1.10.2",
)
...
class TestReadDirectChunk(TestCase):
    def test_read_compressed_offsets(self):
        ...
        with h5py.File(filename, "w") as filehandle:
            ...
            frame_dataset = filehandle.create_dataset(
    ...
    def test_read_uncompressed_offsets(self):
        ...
        with h5py.File(filename, "w") as filehandle:
            ...
            dataset.id.write_direct_chunk(
                (0, 0, 0), frame.tobytes(), filter_mask=DISABLE_ALL_FILTERS
            )
    ...
    def test_read_write_chunk(self):
        ...
        with h5py.File(filename, "w") as filehandle:
            ...
            # copy the data
            dataset.id.write_direct_chunk(
                (0, 0), compressed_frame, filter_mask=filter_mask
            )
