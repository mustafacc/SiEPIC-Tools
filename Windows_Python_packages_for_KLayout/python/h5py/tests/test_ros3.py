...
def test_ros3():
    ...
    with h5py.File(
        "https://dandiarchive.s3.amazonaws.com/ros3test.hdf5", "r", driver="ros3"
    ) as f:
        assert f
...
def test_ros3_s3_fails():
    ...
    with pytest.raises(
        ValueError, match=r"^foo://wrong/scheme: S3 location must begin with"
    ):
...
def test_ros3_s3uri():
    ...
    with h5py.File(
        "s3://dandiarchive/ros3test.hdf5", "r", driver="ros3", aws_region=b"us-east-2"
    ) as f:
        assert f
        assert "mydataset" in f.keys()
        assert f["mydataset"].shape == (100,)
