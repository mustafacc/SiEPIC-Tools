...
class TestLibver(TestCase):
    ...
    def test_libver(self):
        ...
        self.assertEqual(
            (h5f.LIBVER_EARLIEST, h5f.LIBVER_LATEST), plist.get_libver_bounds()
        )

    @ut.skipIf(version.hdf5_version_tuple < (1, 10, 2), "Requires HDF5 1.10.2 or later")
    def test_libver_v18(self):
        ...
        self.assertEqual(
            (h5f.LIBVER_EARLIEST, h5f.LIBVER_V18), plist.get_libver_bounds()
        )
...
class TestDA(TestCase):
    ...
    @ut.skipIf(version.hdf5_version_tuple < (1, 8, 17), "Requires HDF5 1.8.17 or later")
...
class TestFA(TestCase):
    ...
    @ut.skipUnless(
        version.hdf5_version_tuple >= (1, 12, 1)
        or (
            version.hdf5_version_tuple[:2] == (1, 10)
            and version.hdf5_version_tuple[2] >= 7
        ),
    ...
    def test_set_file_locking(self):
        ...
        self.assertEqual(
            (use_file_locking, ignore_when_disabled), falist.get_file_locking()
        )
