...
class TestCreateShape(BaseDataset):
    ...
    def test_long_double(self):
        ...
        if platform.machine() in ["ppc64le"]:
            pytest.xfail(
                "Storage of long double deactivated on %s" % platform.machine()
            )
...
class TestCreateData(BaseDataset):
    ...
    def test_create_bytestring(self):
        ...
        check_vlen_ascii(
            self.f.create_dataset("d", data=np.array([b"abc", b"def"], dtype=object))
        )
    ...
    def test_create_strings(self):
        ...
        check_vlen_utf8(
            self.f.create_dataset("d", data=np.array(["abc", "def"], dtype=object))
        )
...
class TestReadDirectly:
    ...
    def test_read_direct(
        self, writable_file, source_shape, dest_shape, source_sel, dest_sel
    ):
        source_values = np.arange(np.product(source_shape), dtype="int64").reshape(
            source_shape
        )
    ...
    def test_no_sel(self, writable_file):
        dset = writable_file.create_dataset(
            "dset", (10,), data=np.arange(10, dtype="int64")
        )
...
class TestWriteDirectly:
    ...
    def test_write_direct(
        ...
        dset = writable_file.create_dataset(
            "dset", dest_shape, dtype="int32", fillvalue=-1
        )
...
class TestCreateChunked(BaseDataset):
    ...
    def test_scalar_assignment(self):
        ...
        dset = self.f.create_dataset(
            "foo", shape=(3, 50, 50), dtype=np.int32, chunks=(1, 50, 50)
        )
...
class TestCreateFillvalue(BaseDataset):
    ...
    def test_exc(self):
        ...
        with self.assertRaises(ValueError):
            dset = self.f.create_dataset(
                "foo", (10,), dtype=[("a", "i"), ("b", "f")], fillvalue=42
            )
...
class TestCreateGzip(BaseDataset):
    ...
    def test_gzip(self):
        ...
        dset = self.f.create_dataset(
            "foo", (20, 30), compression="gzip", compression_opts=9
        )
    ...
    def test_gzip_exc(self):
        ...
        with self.assertRaises(ValueError):
            self.f.create_dataset(
                "foo", (20, 30), compression="gzip", compression_opts=14
            )
    ...
    def test_compression_number_invalid(self):
        ...
        try:
            ...
            with self.assertRaises(IndexError):
                self.f.create_dataset(
                    "foo", (20, 30), compression=h5py.h5z.FILTER_DEFLATE
                )
        finally:
            h5py._hl.dataset._LEGACY_GZIP_COMPRESSION_VALS = original_compression_vals
...
class TestCreateLZF(BaseDataset):
    ...
    def test_lzf_exc(self):
        ...
        with self.assertRaises(ValueError):
            self.f.create_dataset(
                "foo", (20, 30), compression="lzf", compression_opts=4
            )
...
class TestCreateSZIP(BaseDataset):
    ...
    def test_szip(self):
        ...
        dset = self.f.create_dataset(
            "foo", (20, 30), compression="szip", compression_opts=("ec", 16)
        )
...
class TestExternal(BaseDataset):
    ...
    def test_contents(self):
        ...
        if h5py.version.hdf5_version_tuple >= (1, 10, 0):
            efile_prefix = pathlib.Path(
                dset.id.get_access_plist().get_efile_prefix().decode()
            ).as_posix()
    ...
    def test_contents_efile_prefix(self):
        ...
        dset = self.f.create_dataset(
            ...
            dtype=testdata.dtype,
            external=external,
            efile_prefix=os.path.dirname(ext_file),
        )
        dset[...] = testdata
        ...
        dset2 = self.f.require_dataset(
            "foo", shape, testdata.dtype, efile_prefix=os.path.dirname(ext_file)
        )
...
class TestStrings(BaseDataset):
    ...
    def test_fixed_utf8(self):
        ...
        np.testing.assert_array_equal(
            ds[:8], np.array([s.encode("utf-8")] * 8, dtype="S")
        )
...
class TestVlen(BaseDataset):
    ...
    def test_multidim(self):
        ...
        ds[0, 0] = np.arange(1)
        ds[:, :] = np.array(
            [[np.arange(3), np.arange(2)], [np.arange(1), np.arange(2)]], dtype=object
        )
        ds[:, :] = np.array(
            [[np.arange(2), np.arange(2)], [np.arange(2), np.arange(2)]]
        )
...
@ut.skipUnless(
    h5py.version.hdf5_version_tuple >= (1, 10, 5), "chunk info requires  HDF5 >= 1.10.5"
)
...
@ut.skipUnless(
    h5py.version.hdf5_version_tuple >= (1, 12, 3)
    or (
        h5py.version.hdf5_version_tuple >= (1, 10, 10)
        and h5py.version.hdf5_version_tuple < (1, 10, 99)
    ),
...
class TestCommutative(BaseDataset):
    ...
    def test_numpy_commutative(
        ...
        shape = (100, 1)
        dset = self.f.create_dataset(
            "test", shape, dtype=float, data=np.random.rand(*shape)
        )
...
class TestVirtualPrefix(BaseDataset):
    ...
    @ut.skipIf(
        version.hdf5_version_tuple < (1, 10, 2),
    ...
    def test_virtual_prefix_create(self):
        ...
        virtual_prefix_readback = pathlib.Path(
            dset.id.get_access_plist().get_virtual_prefix().decode()
        ).as_posix()
    ...
    def test_virtual_prefix_require(self):
        virtual_prefix = "/path/to/virtual"
        dset = self.f.require_dataset(
            "foo", (10, 3), "f", virtual_prefix=virtual_prefix
        )
        virtual_prefix_readback = pathlib.Path(
            dset.id.get_access_plist().get_virtual_prefix().decode()
        ).as_posix()
