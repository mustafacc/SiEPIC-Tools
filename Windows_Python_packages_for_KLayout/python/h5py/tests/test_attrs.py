...
class TestTrackOrder(BaseAttrs):
    ...
    @ut.skipUnless(
        h5py.version.hdf5_version_tuple >= (1, 10, 6), "HDF5 1.10.6 required"
    )
    ...
    def test_track_order(self):
        ...
        self.assertEqual(list(attrs), [str(i) for i in range(100)])

    def test_no_track_order(self):
        attrs = self.fill_attrs(track_order=False)  # name alphanumeric
        self.assertEqual(list(attrs), sorted([str(i) for i in range(100)]))
