...
UNSUPPORTED_LONG_DOUBLE_TYPES = ("float96", "float128", "complex192", "complex256")
...
class TestVlen(TestCase):
    ...
    def test_vlen_enum(self):
        ...
        self.assertEqual(
            h5py.check_enum_dtype(h5py.check_vlen_dtype(dt1)),
    ...
    def test_write_empty_vlen(self):
        ...
        with h5py.File(fname, "w") as f:
            ...
            dset = f.create_dataset(
                "test",
                data=d,
                dtype=[("a", "|V16"), ("b", h5py.special_dtype(vlen=np.float_))],
            )
...
class TestOffsets(TestCase):
    ...
    def test_compound_vlen(self):
        ...
        for np_align in (False, True):
            dt = np.dtype(
                [("a", eidt), ("foo", vidt), ("bar", vidt), ("switch", eidt)],
                align=np_align,
            )
            ...
            for logical in (False, True):
                if logical and np_align:
                    ...
                    self.assertRaises(
                        TypeError, h5py.h5t.py_create, dt, logical=logical
                    )
                else:
    ...
    def test_compound_robustness(self):
        ...
        f1 = np.array(
            [1 + i * 4 for i in range(data.shape[0])], dtype=dt.fields["f1"][0]
        )
        f2 = np.array(
            [2 + i * 4 for i in range(data.shape[0])], dtype=dt.fields["f2"][0]
        )
        f3 = np.array(
            [3 + i * 4 for i in range(data.shape[0])], dtype=dt.fields["f3"][0]
        )
        f0c = 3.14
        data["f0"] = f0c
        data["f3"] = f3
    ...
    def test_float_round_tripping(self):
        dtypes = set(
            f
            for f in np.sctypeDict.values()
            if (np.issubdtype(f, np.floating) or np.issubdtype(f, np.complexfloating))
        )
        ...
        dtype_dset_map = {
            str(j): d for j, d in enumerate(dtypes) if d not in unsupported_types
        }
...
class TestBitfield(TestCase):
    ...
    def test_b8_bool_compound_mixed_types(self):
        ...
        self._test_b8(
            arr1, expected_default_cast_dtype=np.dtype([("x", "u1"), ("y", "<f8")])
        )
        self._test_b8(
            arr1,
    ...
    def _test_b8(self, arr1, expected_default_cast_dtype, cast_dtype=None):
        ...
        with h5py.File(path, "r") as f:
            ...
            self.assertArrayEqual(
                arr2, arr1.astype(expected_default_cast_dtype, copy=False)
            )
