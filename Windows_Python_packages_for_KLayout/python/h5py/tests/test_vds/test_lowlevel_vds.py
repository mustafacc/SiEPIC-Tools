...
@ut.skipUnless(
    h5.version.hdf5_version_tuple >= (1, 9, 233), "VDS requires HDF5 >= 1.9.233"
)
        ...
        with h5.File(self.outfile, "w", libver="latest") as f:
            ...
            for foo in self.fname:
                ...
                src_dspace.select_hyperslab(
                    start=(0, 0, 0), stride=(1, 1, 1), count=(1, 1, 1), block=src_shape
                )

                virt_dspace.select_hyperslab(
                    start=(k, 0, 0), stride=(1, 1, 1), count=(1, 1, 1), block=src_shape
                )
...
class ExcaliburData:
    ...
    def sensor_module_dimensions(self):
        ...
        y_pixels = (
            ...
            * self.FEM_CHIPS_PER_STRIPE_Y
            * self.FEM_STRIPES_PER_MODULE
        )
        return (
            y_pixels,
            x_pixels,
...
class TestExcaliburLowLevel(ut.TestCase):
    def create_excalibur_fem_stripe_datafile(
        self, fname, nframes, excalibur_data, scale
    ):
        ...
        with h5.File(fname, "w", libver="latest") as f:
            dset = f.create_dataset(
                "data", shape=shape, maxshape=max_shape, chunks=chunk, dtype="uint16"
            )
            for data_value_index in np.arange(nframes):
                dset[data_value_index] = excalibur_data.generate_fem_stripe_image(
                    data_value_index * scale
                )
    ...
    def test_excalibur_low_level(self):
        ...
        vdset_shape = (
            5,
            excalibur_data.fem_stripe_dimensions[0] * len(self.fname)
            + (10 * (len(self.fname) - 1)),
        ...
        with h5.File(self.outfile, "w", libver="latest") as f:
            # Create the source dataset dataspace
            src_dspace = h5.h5s.create_simple(
                vdset_stripe_shape, vdset_stripe_max_shape
            )
...
@ut.skipUnless(
    h5.version.hdf5_version_tuple >= (1, 10, 2),
