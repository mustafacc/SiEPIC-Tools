...
class TestCache(TestCase):
    ...
    def test_w0(self):
        ...
        self.assertEqual(
            list(f.id.get_access_plist().get_cache()), [0, 521, 1048576, 0.25]
        )
...
class TestFileObj(TestCase):
    ...
    def check_read(self, fileobj):
        ...
        self.assertEqual(list(f["test"][:]), list(range(12)))
        self.assertRaises(
            Exception, f.create_dataset, "another.test", data=list(range(3))
        )
        f.close()
...
class TestTrackOrder(TestCase):
    ...
    def test_no_track_order(self):
        ...
        self.populate(f)
        self.assertEqual(list(f), sorted([str(i) for i in range(100)]))
...
class TestFileMetaBlockSize(TestCase):
    ...
    def test_file_create_with_meta_block_size_4096(self):
        ...
        with File(
            self.mktemp(), "w", meta_block_size=meta_block_size, libver="latest"
        ) as f:
            f["test"] = 5
    ...
    def test_file_create_with_meta_block_size_512(self):
        ...
        libver = "latest"
        with File(
            self.mktemp(), "w", meta_block_size=meta_block_size, libver=libver
        ) as f:
            f["test"] = 3
            self.assertEqual(f.meta_block_size, meta_block_size)
            # Equality is expected for HDF5 1.10
