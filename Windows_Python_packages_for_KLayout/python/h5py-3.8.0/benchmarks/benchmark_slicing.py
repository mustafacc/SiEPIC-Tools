...
class SlicingBenchmark:
    ...
    def __init__(
        ...
        self.needed_memory = (
            self.size ** (self.ndim - 1) * self.dtype.itemsize * self.chunk
        )
    ...
    def setup(self):
        ...
        logger.info(
            "Total size: %i^%i volume size: %.3fGB, Needed memory: %.3fGB",
            self.size,
            self.ndim,
            self.total_size / 1e9,
            self.needed_memory / 1e9,
        )
        ...
        if self.precision and self.dtype.char in "df":
            if self.dtype.itemsize == 4:
                ...
                logger.warning(
                    "Precision reduction: only float32 and float64 are supported"
                )
        else:
            self.precision = 0
        t0 = time.time()
        with h5py.File(self.filename, "w") as h:
            ds = h.create_dataset(self.h5path, shape, chunks=chunks, **self.compression)
            for i in range(0, self.size, self.chunk):
                x, y, z = numpy.ogrid[i : i + self.chunk, : self.size, : self.size]
                data = (numpy.sin(x / 3) * numpy.sin(y / 5) * numpy.sin(z / 7)).astype(
                    self.dtype
                )
                if self.precision:
        ...
        logger.info(
            "Compression: %.3f time %.3fs uncompressed data saving speed %.3f MB/s effective write speed  %.3f MB/s ",
            self.total_size / filesize,
            dt,
            self.total_size / dt / 1e6,
            filesize / dt / 1e6,
        )
    ...
    def time_sequential_reads(self, nb_read=64):
        ...
        where = [
            [(i * (self.chunk + 1 + j)) % self.size for j in range(self.ndim)]
        ...
        dt = t1 - t0
        logger.info(
            "Time for reading %sx%s slices: %.3fs fps: %.3f "
            % (self.ndim, nb_read, dt, self.ndim * nb_read / dt)
            + "Uncompressed data read speed %.3f MB/s"
            % (self.ndim * nb_read * self.needed_memory / dt / 1e6)
        )
        return dt
    ...
    def time_threaded_reads(self, nb_read=64, nthreads=multiprocessing.cpu_count()):
        ...
        dt = t1 - t0
        logger.info(
            "Time for %s-threaded reading %sx%s slices: %.3fs fps: %.3f "
            % (nthreads, self.ndim, nb_read, dt, self.ndim * nb_read / dt)
            + "Uncompressed data read speed %.3f MB/s"
            % (self.ndim * nb_read * self.needed_memory / dt / 1e6)
        )
        return dt
