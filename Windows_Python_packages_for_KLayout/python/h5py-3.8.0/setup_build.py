...
if os.environ.get("H5PY_SYSTEM_LZF", "0") == "1":
    EXTRA_LIBRARIES = {"h5z": ["lzf"]}
else:
    COMPILER_SETTINGS["include_dirs"] += [localpath("lzf/lzf")]

    EXTRA_SRC["h5z"] += [localpath("lzf/lzf/lzf_c.c"), localpath("lzf/lzf/lzf_d.c")]
...
class h5py_build_ext(build_ext):
    ...
    def run(self):
        ...
        complex256_support = (
            ...
            and os.environ.get("CIBW_ARCHS_MACOS") != "arm64"
        )
        ...
        if (
            not op.isfile(defs_file)
            or os.stat(func_file).st_mtime > os.stat(defs_file).st_mtime
        ):
