...
def select_paths_m(layers, cell=None, verbose=None):
    ...
    if selection == []:
        for (
            ...
            layers
        ):  # run through all the layers inputted into the function (allows for multiple path layer selections)
            itr = cell.begin_shapes_rec(ly.layer(layer))
            while not (itr.at_end()):
                if verbose:
                    print("SiEPIC.utils.select_paths: itr: %s" % itr)
                if (
                    itr.shape().is_path() and not itr.cell().pcell_parameters()
...
def select_wireguides(cell=None):
    ...
    if selection == []:
        for instance in cell.each_inst():
            if (
                instance.cell.basic_name() == "Wireguide"
            ):  # replace "Waveguide" with "Wireguide"
                selection.append(pya.ObjectInstPath())
                selection[-1].top = cell.cell_index()
        ...
        lv.object_selection = [
            o
            for o in selection
            if o.is_cell_inst() and o.inst().cell.basic_name() == "Wireguide"
        ]
...
# 5) Changed all "waveguide" names and variables to "wireguides"
def path_to_wireguide(
    cell=None, lv_commit=True, verbose=False, select_wireguides=False
):
    ...
    selected_paths = select_paths_m(
        layers_to_select, cell, verbose=verbose
    )  # find all paths on the selected layers

    if verbose:
        print(
            "SiEPIC.scripts path_to_wireguide(): selected_paths = %s" % selected_paths
        )
    selection = []
    ...
    for obj in selected_paths:
        ...
        if obj.shape.property(
            ...
            while not iter.at_end():
                lp = iter.current()
                if (
                    lp.cellview() == obj.cv_index and lp.layer_index() == obj.layer
                ):  # find specified layer within the layer loop
        ...
        width_devrec = (
            params["width"] + _globals.WG_DEVREC_SPACE * 2
        )  # get DevRec width based on the wireguide width found earlier
        try:
...
def wireguide_to_path(cell=None):
    ...
    for obj in wireguides:
        ...
        if KLAYOUT_VERSION > 24:
            ...
            path = pya.Path()
            path.width = (
                wireguide.cell.pcell_parameters_by_name()["width"] / TECHNOLOGY["dbu"]
            )
            pts = []
            ...
            if o_selection.is_cell_inst() == False:
                ...
                pin_pairs = sorted(
                    [
                        ...
                        if (abs(pin_t.rotation - pin_s.rotation) % 360 - 180) < 1
                        and pin_t.type == _globals.PIN_TYPES.ELECTRICAL
                        and pin_s.type == _globals.PIN_TYPES.ELECTRICAL
                    ],
