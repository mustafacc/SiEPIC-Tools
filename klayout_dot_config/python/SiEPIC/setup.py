...
def registerMenuItems():
    ...
    path = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "files", "INTERCONNECT_icon.png"
    )
    path_flv = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "files", "flv_icon.png"
    )
    ...
    if not (menu.is_menu(s1)):
        menu.insert_menu(
            "help_menu", s1, "SiEPIC %s" % SiEPIC.__init__.__version__ + extra
        )
...
def registerKeyBindings():
    ...
    config = "".join("{}:{};".format(key, val) for key, val in sorted(mapping.items()))[
        :-1
    ]
