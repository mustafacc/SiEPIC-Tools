...
try:
    ...
    try:
        ...
        install = pya.MessageBox.warning(
            "Install package?",
            "Install package 'pyparsing' using pip? [required for Lumerical FDTD]",
...
def run_FDTD(verbose=False):
    ...
    if not lumapi:
        print(
            "SiEPIC.lumerical.interconnect.run_FDTD: lumapi not loaded; reloading load_lumapi."
        )
        import sys
...
def generate_component_sparam(
    ...
    if do_simulation:
        ...
        print(
            "%s, %s, %s, %s"
            % (
                devrec_box.left * dbum,
                devrec_box.right * dbum,
                devrec_box.bottom * dbum,
                devrec_box.top * dbum,
            )
        )
        ...
        print(
            "mode_selection_index %s, symmetry: %s" % (mode_selection_index, Z_symmetry)
        )
        if fdtd_extra_ == True:
            fdtd_extra = 200e-9
        else:
        ...
        FDTDxmin, FDTDxmax, FDTDymin, FDTDymax = (
            (devrec_box.left) * dbum - fdtd_extra,
            (devrec_box.right) * dbum + fdtd_extra,
            (devrec_box.bottom) * dbum - fdtd_extra,
            (devrec_box.top) * dbum + fdtd_extra,
        )
      ...
      set('z min bc','%s'); set('z max bc','%s'); "
            % (
                ...
                FDTDzspan,
                Z_symmetry,
                FDTD_settings["Initial_Z-Boundary-Conditions"],
            ),
        )
        lumapi.evalScript(
            _globals.FDTD,
      ...
      ?'FDTD solver with mesh override added'; "
            % (
                wavelength_start,
                wavelength_stop,
                FDTD_settings["frequency_points_monitor"],
                sim_time,
                FDTD_settings["thickness_Si"] / 4,
                FDTD_settings["thickness_Si"] / 2,
            ),
        )
      ...
      ?'oxide added';    "
            % (
                ...
                FDTDymin - 1e-6,
                FDTDymax + 1e-6,
                -FDTD_settings["thickness_BOX"] - FDTD_settings["thickness_Si"] / 2,
                -FDTD_settings["thickness_Si"] / 2,
      ...
      ?'oxide added';    "
            % (
                ...
                FDTDymax + 1e-6,
                -FDTD_settings["thickness_Si"] / 2,
                FDTD_settings["thickness_Clad"] - FDTD_settings["thickness_Si"] / 2,
                FDTD_settings["material_Clad"],
            ),
        )
        ...
        def send_polygons_to_FDTD(polygons):
            ...
            lumapi.evalScript(
                _globals.FDTD,
                "switchtolayout; select('polygons'); delete; addgroup; set('name','polygons'); set('x',0); set('y',0);",
            )
            for i in range(0, len(polygons_vertices)):
                print(
                    "  polygons' vertices (%s): %s"
                    % (len(polygons_vertices[i]), polygons_vertices[i])
                )
                lumapi.putMatrix(
                    _globals.FDTD, "polygon_vertices", np.array(polygons_vertices[i])
                )
                lumapi.evalScript(
                    _globals.FDTD,
                    " \
        ...
        ?'Added pin: %s, set %s to PML'; "
                % (
                    p.pin_name,
                    p.direction,
                    FDTD_settings["frequency_points_expansion"],
                    mode_selection,
                    port_dict[p.rotation],
                    p.pin_name,
                    port_dict[p.rotation],
                ),
            )
        ...
        if not "win" in sys.platform:  # Windows getVar ("E") doesn't work.
            ...
            ?'Selected pin: %s'; "
                        % (p.pin_name, p.pin_name, m, p.pin_name),
                    )
      ...
      save('%s');\
      ?'FDTD Z-span updated to %s'; "
            % (FDTDzspan, fsp_filename, FDTDzspan),
        )
        ...
        def FDTD_run_Sparam_simple(
            pins, in_pin=None, out_pins=None, modes=[1], plots=False
        ):
            if verbose:
            ...
            Sparam=S_%s_%s= Port_%s.%s/Port_%s.%s;  \
             "
                        % (
                            ...
                            in_pin.pin_name,
                            p.pin_name,
                            "b" if p.direction == "Forward" else "a",
                            in_pin.pin_name,
                            "a" if in_pin.direction == "Forward" else "b",
                        ),
                    )
                    ...
                    if plots:
                        if verbose:
                            print(
                                " Plot S_%s_%s Sparam" % (p.pin_name, in_pin.pin_name)
                            )
                        lumapi.evalScript(
                            _globals.FDTD,
                            " \
        ...
        for m in mode_selection_index:
            Sparam_pin_max_modes.append(
                np.amax(
                    np.absolute(np.array(Sparams)[:, :, mode_selection_index.index(m)]),
                    axis=1,
                ).argmax()
                + 1
            )
            Mean_IL_best_port.append(
                -10
                * np.log10(
                    np.mean(
                        np.absolute(Sparams)[
                            Sparam_pin_max_modes[-1] - 1,
                            :,
        ...
        for m in mode_selection_index:
            info_text += "mode %s, S_%s_%s has %s dB average insertion loss\n" % (
                m,
                pins[Sparam_pin_max_modes[mode_selection_index.index(m)]].pin_name,
                in_pin.pin_name,
        ...
        if FDTD_settings["convergence_tests"]:
          ...
          ?'FDTD Z-span: %s, rms error from previous: %s (convergence testing until < %s)'; "
                ...
                if len(convergence) > 2:
                    test_rms = np.polyfit(
                        np.array(convergence)[-3:, 0], np.array(convergence)[-3:, 1], 1
                    )
                    if verbose:
                        print(
                            "  convergence rms trend: %s; fit data: %s"
                    ...
                    if test_rms[0] > 0:
                        ...
                        test_converged = True
                        FDTDzspan += (
                            -2 * FDTD_settings["convergence_test_span_incremement"]
                        )
        ...
        if FDTD_settings["Perform-quick-corner-analysis"]:
            for w in [
                -FDTD_settings["width_Si_corners"],
                FDTD_settings["width_Si_corners"],
            ]:
                polygons_w = [
                    p for p in pya.Region(polygons).sized(w / 2 * 1e9).each_merged()
                ]
                send_polygons_to_FDTD(polygons_w)
                for h in [
                    -FDTD_settings["thickness_Si_corners"],
                    FDTD_settings["thickness_Si_corners"],
                ]:
                    lumapi.evalScript(
                        _globals.FDTD,
      ...
      ?'FDTD mesh accuracy updated %s, Z boundary conditions: %s'; "
        ...
        ?'updated pin: %s'; "
                % (p.pin_name, FDTD_settings["frequency_points_expansion"], p.pin_name),
            )
      ...
      "
        ...
        pin_h0, pin_w0 = str(round(FDTD_settings["thickness_Si"], 9)), str(
            round(pins[0].path.width * dbum, 9)
        )
        file_sparam = os.path.join(
            _globals.TEMP_FOLDER,
            "%s_t=%s_w=%s.dat" % (component.instance, pin_h0, pin_w0),
        )
  ...
  <extracted> \n\
      <value name="sparam" type="string">%s</value> \n\
        ...
        if FDTD_settings["Perform-final-corner-analysis"]:
            lumapi.evalScript(
                _globals.FDTD, "leg=cell(4*%s); li=0; \n" % (len(mode_selection_index))
            ...
            for w in [
                ...
                for h in [
                    ...
                    # export S-parameter data to file named xxx.dat to be loaded in INTERCONNECT
                    pin_h, pin_w = str(
                        round(FDTD_settings["thickness_Si"] + h, 9)
                    ), str(round(pins[0].path.width * dbum + w, 9))
                    file_sparam = os.path.join(
                        _globals.TEMP_FOLDER,
                        "%s_t=%s_w=%s.dat" % (component.instance, pin_h, pin_w),
                    )
                    files_sparam.append(file_sparam)
                    lumapi.evalScript(
                        _globals.FDTD,
                  ...
                  "
                    ...
                    for m in mode_selection_index:
                      ...
                      leg{li} = 'S_%s_%s:%s - %s, %s'; \
                       "
                            % (
                                Sparam_pin_max_modes[mode_selection_index.index(m)] + 1,
                                pins[
                                    Sparam_pin_max_modes[mode_selection_index.index(m)]
                                ].pin_name,
                                in_pin.pin_name,
                                mode_selection_index.index(m) + 1,
                                pin_h,
                                pin_w,
                            ),
                        )
  ...
  <extracted> \n\
    ...
    if addto_CML:
        ...
        if os.path.exists(svg_filename):
            ...
            print(
                " SiEPIC.lumerical.fdtd.component... missing SVG icon: %s"
                % svg_filename
            )
        ...
        for p in pins:
            ...
            if p.rotation in [0.0, 180.0]:
                location = (
                    1
                    - (p.center.y - component.DevRec_polygon.bbox().bottom + 0.0)
                    / component.DevRec_polygon.bbox().height()
                )
            #      print(" p.y %s, c.bottom %s, location %s: " % (p.center.y,component.polygon.bbox().bottom, location) )
            else:
                location = (
                    p.center.x - component.DevRec_polygon.bbox().left + 0.0
                ) / component.DevRec_polygon.bbox().width()
                print(location)
            t += (
            ...
            t += (
                'connect(component+"::RELAY_%s", "port", component+"::SPAR_1", "opt%s");\n'
                % (count, count)
            )
        ...
        if FDTD_settings["Perform-final-corner-analysis"]:
            ...
            INTC_files = os.path.join(
                INTC_custom, INTC_Lib, "source_data", component.instance
            )
...
def generate_GC_sparam(
    ...
    set('input length',%s);"
            % (
                filename,
                GC_settings["duty_cycle"],
                GC_settings["target_length"],
                GC_settings["etch_depth"],
                GC_settings["pitch"],
        ...
        # set fiber angle
        lumapi.evalScript(
            _globals.FDTD, "select('fiber'); set('theta0',%s);" % (GC_settings["angle"])
        )
    ...
    set('mode selection',%s);"
      ...
      set('mode selection',%s); updateportmodes;"
...
def generate_CDC_bandstructure(
    W_1=450e-9,
    W_2=550e-9,
    dW_1=20e-9,
    dW_2=40e-9,
  ...
  setnamed('::model','bus2_delta',%s); setnamed('::model','ax',%s); setnamed('::model','gap',%s); setnamed('::model','sinusoidal',%s);"
        % (filename, W_2, W_1, dW_2, dW_1, period, gap, sinusoidal),
    )
