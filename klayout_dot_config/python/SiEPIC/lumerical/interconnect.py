...
def run_INTC(verbose=False):
    ...
    if not lumapi:
        print(
            "SiEPIC.lumerical.interconnect.run_INTC: lumapi not loaded; reloading load_lumapi."
        )
        import sys
...
def INTC_installdesignkit(verbose=False):
    ...
    try:
        ...
        library_ids = [
            n
            for n in pya.Library.library_ids()
            if (
                pya.Library.library_by_id(n).technology == TECHNOLOGY["technology_name"]
            )
        ]
    ...
    for i in range(0, len(library_ids)):
        print(
            ...
            % (pya.Library.library_by_id(library_ids[i]).name())
        )
        if "path" in dir(pya.Library.library_by_id(library_ids[i])):
            ...
            for f in os.listdir(path):
                if f.lower().endswith(".cml"):
                    ...
                    # install CML
                    print(
                        "Lumerical INTC, installdesignkit ('%s', '%s', true);"
                        % (ff, dir_path)
                    )
                    lumapi.evalScript(
                        _globals.INTC,
                        "installdesignkit ('%s', '%s', true);" % (ff, dir_path),
                    )
    ...
    if "INTC_CMLs_path" in TECHNOLOGY and TECHNOLOGY["INTC_CML_path"]:
        if (
            not (
                "design kits::"
                + TECHNOLOGY["technology_name"].lower()
                + "::"
                + TECHNOLOGY["INTC_CML_version"].lower().replace(".cml", "").lower()
            )
            in _globals.INTC_ELEMENTS
        ):
            # install CML
            print(
                "Lumerical INTC, installdesignkit ('%s', '%s', true);"
        ...
        for i in range(0, len(TECHNOLOGY["INTC_CMLs_name"])):
            if (
                not (
                    "design kits::"
                    + TECHNOLOGY["INTC_CMLs_name"][i].lower()
                    + "::"
                    + TECHNOLOGY["INTC_CMLs_version"][i]
                    .lower()
                    .replace(".cml", "")
                    .lower()
                )
    ...
    question.setDefaultButton(pya.QMessageBox.Yes)
    question.setText(
        "Do you wish to see all the components in %s library?"
        % TECHNOLOGY["technology_name"]
    )
    ...
    for i in range(0, num):
        lumapi.evalScript(
            _globals.INTC,
            "a=addelement('%s'); setposition(a,%s,%s); " % (tech_elements[i], x, y),
        )
        y += 250
...
def INTC_loaddesignkit(folder_CML, verbose=False):
    ...
    if not next(os.walk(folder_CML))[1]:
        raise Exception(
            "SiEPIC.lumerical.interconnect.INTC_loaddesignkit: Cannot find Lumerical INTERCONNECT Compact Model Library (CML) folder, which should be in the PDK folder under the CML subfolder."
        )
    ...
    if not lumapi:
        raise Exception(
            "SiEPIC.lumerical.interconnect.INTC_loaddesignkit: Cannot load Lumerical INTERCONNECT and Python integration (lumapi)."
        )
    ...
    for folder_name in next(os.walk(folder_CML))[1]:
        if not ("design kits::" + folder_name.lower() + "::" in _globals.INTC_ELEMENTS):
            ...
            lumapi.evalScript(
                _globals.INTC,
                "loaddesignkit ('%s', '%s');" % (folder_name, folder_path),
            )
            new_loaded = True
    ...
    if "design kits::" + folder_name.lower() + "::" in _globals.INTC_ELEMENTS:
        integration_success_message = (
            "message('KLayout-Lumerical INTERCONNECT integration successful, for Compact Model Libraries installed in Element Library | Design kits: %s.\n"
...
def Setup_Lumerical_KLayoutPython_integration(verbose=False):
    ...
    if lv == None:
        raise UserWarning(
            "To install the Compact Model Library, first, please create a new layout and select the desired technology:\n  Menu: File > New Layout, and a Technology.\nThen repeat."
        )
...
def INTC_commandline(filename2):
    ...
    if sys.platform.startswith("linux"):
        ...
        file_path_a = (
            "C:\\Program Files\\Lumerical\\INTERCONNECT\\bin\\interconnect.exe"
        )
        file_path_b = (
            "C:\\Program Files (x86)\\Lumerical\\INTERCONNECT\\bin\\interconnect.exe"
        )
        ...
        if os.path.isfile(file_path_a) == True:
            ...
            warning_window.setStandardButtons(
                pya.QMessageBox.Yes | pya.QMessageBox.Cancel
            )
...
def component_simulation(verbose=False, simulate=True):
    ...
    for obj in selected_instances:
        ...
        if not c.has_model():
            if len(selected_instances) == 0:
                error.setText(
                    "Error: Component '%s' does not have a compact model. Cannot perform simulation."
                    % c
                )
                continue
        ...
        pin_names = [
            ...
            or p.type == _globals.PIN_TYPES.OPTICALIO
        ]
        if not pin_names:
            continue
        pin_injection = pya.InputDialog.ask_item(
            "Pin selection",
            "Choose one of the pins in component '%s' to inject light into."
            % c.component,
            pin_names,
            0,
        )
        ...
        if verbose:
            print(
                "Pin selected from InputDialog = %s, for component '%s'."
                % (pin_injection, c.component)
            )
        ...
        text_main = (
            "* Spice output from KLayout SiEPIC-Tools v%s, %s technology (SiEPIC.lumerical.interconnect.component_simulation), %s.\n\n"
            % (
                SiEPIC.__version__,
                TECHNOLOGY["technology_name"],
        ...
        for p in c.pins:
            if p.type == _globals.PIN_TYPES.ELECTRICAL:
                ...
                Vn += 1
            if (
                p.type == _globals.PIN_TYPES.OPTICAL
                or p.type == _globals.PIN_TYPES.OPTICALIO
            ):
    ...
    """
        ...
        if 0:
            for i in range(0, len(pin_names)):
                ...
                text_lsf += (
                    'if (h%s>0) { visualize(getresult("ONA_1", "input %s/mode 1/gain")); } \n'
                    % (i + 1, i + 1)
                )
        if 1:
            text_lsf += 't = "";\n'
            for i in range(0, len(pin_names)):
                ...
                text_lsf += (
                    'if (h%s>0) { t%s = getresult("ONA_1", "input %s/mode 1/gain"); t=t+"t%s,"; } \n'
                    % (i + 1, i + 1, i + 1, i + 1)
                )
...
def circuit_simulation_toolbar():
    circuit_simulation(
        verbose=False, opt_in_selection_text=[], matlab_data_files=[], simulate=True
    )
...
def circuit_simulation(
    ...
    for i in range(1, num_detectors + 1):
        if matlab_data_files:
            ...
            text_lsf += (
                't%s.addattribute("Simulation, Detector %s",getresultdata("ONA_1", "input %s/mode 1/gain"));\n'
                % (i, i, i)
            )
        else:
    ...
    if matlab_data_files:
        for m in matlab_data_files:
            if ".mat" in m:
                ...
                text_lsf += (
                    'm%s.addparameter("wavelength",scandata.wavelength*%s);\n'
                    % (m_count, wavelenth_scale)
                )
                for d in detector_list:
                    text_lsf += (
                        'm%s.addattribute("Measured: %s",scandata.power(:,%s));\n'
                        % (m_count, tail, d)
                    )
...
def circuit_simulation_monte_carlo(
    params=None,
    topcell=None,
    verbose=True,
    ...
    if params is None:
        params = _globals.MC_GUI.get_parameters()
    if params is None:
        pya.MessageBox.warning(
            "No MC parameters",
            "No Monte Carlo parameters. Cancelling.",
            pya.MessageBox.Cancel,
        )
        return
    print(params)
    ...
    if int(params["num_wafers"]) < 1:
        pya.MessageBox.warning(
            "Insufficient number of wafers",
            "The number of wafers for Monte Carlo simulations need to be 1 or more.",
            pya.MessageBox.Cancel,
        )
        return
    if int(params["num_dies"]) < 1:
        pya.MessageBox.warning(
            "Insufficient number of dies",
            "The number of die per wafer for Monte Carlo simulations need to be 1 or more.",
            pya.MessageBox.Cancel,
        )
        return
    ...
    text_lsf += (
        'addproperty("::Root Element::%s", "MC_uniformity_thickness", "wafer", "Matrix");\n'
        % circuit_name
    )
    text_lsf += (
        'addproperty("::Root Element::%s", "MC_uniformity_width", "wafer", "Matrix");\n'
        % circuit_name
    )
    text_lsf += (
        'addproperty("::Root Element::%s", "MC_grid", "wafer", "Number");\n'
        % circuit_name
    )
    text_lsf += (
        'addproperty("::Root Element::%s", "MC_resolution_x", "wafer", "Number");\n'
        % circuit_name
    )
    text_lsf += (
        'addproperty("::Root Element::%s", "MC_resolution_y", "wafer", "Number");\n'
        % circuit_name
    )
    text_lsf += (
        'addproperty("::Root Element::%s", "MC_non_uniform", "wafer", "Number");\n'
        % circuit_name
    )
    ...
    text_lsf += (
        "n_wafer = %s;  \n" % params["num_wafers"]
    )  #  GUI INPUT: Number of testing wafer
    text_lsf += (
        "n_die = %s;  \n" % params["num_dies"]
    )  #  GUI INPUT: Number of testing die per wafer
    text_lsf += "kk = 1;  \n"
    ...
    for i in range(0, num_detectors):
        text_lsf += (
            'mc%s = matrixdataset("mc%s"); # initialize visualizer data, mc%s \n'
            % (i + 1, i + 1, i + 1)
        )
    ...
    text_lsf += (
        " wafer_cl_width = %s;  \n" % params["waf_var"]["width"]["corr_len"]
    )  # [m],  GUI INPUT: wafer correlation length
    text_lsf += (
        " wafer_cl_thickness = %s;  \n" % params["waf_var"]["height"]["corr_len"]
    )  # [m],  GUI INPUT: wafer correlation length
    ...
    text_lsf += (
        " wafer_RMS_w = %s;     \n" % params["waf_var"]["width"]["std_dev"]
    )  # [nm], GUI INPUT: Within wafer Sigma RMS for width
    text_lsf += (
        " wafer_RMS_t = %s;   \n" % params["waf_var"]["height"]["std_dev"]
    )  # [nm], GUI INPUT: Within wafer Sigma RMS for thickness
    ...
    ######################## adjust Wafer mean ###################
    text_lsf += (
        " mean_RMS_w = %s;     \n" % params["waf_to_waf_var"]["width"]["std_dev"]
    )  # [nm], GUI INPUT:  wafer Sigma RMS for width
    text_lsf += (
        " mean_RMS_t = %s;   \n" % params["waf_to_waf_var"]["thickness"]["std_dev"]
    )  # [nm], GUI INPUT:  wafer Sigma RMS for thickness
    ...
    for i in range(0, num_detectors):
        ...
        text_lsf += (
            '  mc%s.addattribute("run", getattribute( getresult("ONA_1", "input %s/mode 1/gain"), getattribute(getresult("ONA_1", "input %s/mode 1/gain")) ) );\n'
            % (i + 1, i + 1, i + 1)
        )
        text_lsf += (
            '  Gain_Data_input%s(1:num_points, kk) = getattribute( getresult("ONA_1", "input %s/mode 1/gain"), getattribute(getresult("ONA_1", "input %s/mode 1/gain")) ); \n'
            % (i + 1, i + 1, i + 1)
        )
    ...
    if params["histograms"]["gain"] == True:
        ...
        text_lsf += "num_hist = max( [ 10, (max(dataset)-min(dataset)) / std(dataset) * 10 ]);\n"
        text_lsf += (
            'histc(dataset, bin_hist, "Gain (dB)", "Count", "Histogram - Peak gain");\n'
        )
        text_lsf += 'legend("Mean: " + num2str(mean(dataset)) + ", Std Dev: " + num2str(std(dataset)));\n'
