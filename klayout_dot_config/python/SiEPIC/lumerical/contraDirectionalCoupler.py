...
def contraDC_model(params, verbose=True):
    ...
    Lambda = np.linspace(
        ...
        num=int(params["wavelength_points"]),
    )
...
def cdc_simulation(verbose=True, FDTD_settings=None):
    ...
    if params["sinusoidal"] == False:
        ...
        sinusoidal = 1
        error.setText(
            "Error: The simulation model currently does not support sinusoidal gratings."
        )
        response = error.exec_()
        return
    ...
    if params["AR"] == False:
        error.setText(
            "Error: The simulation model currently only supports anti-reflection gratings."
        )
        response = error.exec_()
        return
    ...
    params["kappa_contra"] = kappa_contra = get_kappa(
        params["delta_lambda_contra"], params["lambda_contra"], params["ng_contra"]
    )
    params["kappa_self1"] = kappa_self1 = (
        get_kappa(params["delta_lambda_self1"], params["lambda_self1"], params["ng1"])
        / 10
    )
    params["kappa_self2"] = kappa_self2 = (
        get_kappa(params["delta_lambda_self2"], params["lambda_self2"], params["ng2"])
        / 10
    )

    print(
        "Kappa contra = %s, Kappa self 1 = %s, Kappa self 2 = %s"
        % (kappa_contra, kappa_self1, kappa_self2)
    )
