...
try:
    ...
    try:
        ...
        install = pya.MessageBox.warning(
            "Install package?",
            "Install package 'pyparsing' using pip? [required for Lumerical MODE]",
...
def run_MODE(verbose=False):
    ...
    if not lumapi:
        print(
            "SiEPIC.lumerical.mode.run_MODE: lumapi not loaded; reloading load_lumapi."
        )
        import sys
...
def find_neff_supermode(w_1=500e-9, w_2=500e-9, gap=200e-9, pol="TE", verbose=False):
    ...
    lumapi.evalScript(
        _globals.MODE,
        "load('%s'); gap = %s; width_1 = %s; width_2 = %s;" % (filename, gap, w_1, w_2),
    )
...
def dispersion_2WG(params, verbose=False):
    ...
    lumapi.evalScript(
        _globals.MODE,
        "gap = %s; width_1 = %s; width_2 = %s; thick_Si = %s; period = %s;"
        % (
            params["gap"] * 1e-6,
            params["wg1_width"] * 1e-6,
            params["wg2_width"] * 1e-6,
            params["thickness_Si"],
            params["grating_period"] * 1e-6,
        ),
    )

    lumapi.evalScript(
        _globals.MODE,
        "wavelength = %s; wavelength_stop = %s; number_of_plot_points = %s; pol = %s;"
        % (
            params["wavelength_start"],
            params["wavelength_stop"],
            params["wavelength_points"],
            params["pol"],
        ),
    )
...
def cdc_EME(params, verbose=False):
    ...
    lumapi.evalScript(
        _globals.MODE,
        "gap = %s; Wa = %s; Wb = %s; thick_Si = %s; grating_period = %s;"
        % (
            params["gap"] * 1e-6,
            params["wg1_width"] * 1e-6,
            params["wg2_width"] * 1e-6,
    ...
    lumapi.evalScript(
        _globals.MODE,
        "dWa = %s; dWb = %s; number_of_periods=%s"
        % (
            params["corrugation_width1"] * 1e-6,
            params["corrugation_width2"] * 1e-6,
            params["number_of_periods"],
        ),
    )

    lumapi.evalScript(
        _globals.MODE,
        "min_wavelength = %s; max_wavelength = %s; pol = %s;"
        % (params["wavelength_start"], params["wavelength_stop"], params["pol"]),
    )

    lumapi.evalScript(_globals.MODE, "wl_ref = %s;" % (params["contra_lambda"]))

    lumapi.evalScript(
        _globals.MODE,
        "load('%s'); %s; ?'delta_lambda = '+num2str(delta_lambda)"
