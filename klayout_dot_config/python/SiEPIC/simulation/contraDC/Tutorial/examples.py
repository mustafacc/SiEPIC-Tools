...
def examples(num):
    ...
    if num == 1:
        ...
        gd = go.Scatter(
            x=device.wavelength * 1e9,
            y=device.group_delay * 1e12,
            mode="lines",
            name="Group delay",
        )
        layout = go.Layout(
    ...
    if num == 2:
        ...
        layout = go.Layout(
            title="Contra-directional coupler device",
            xaxis=dict(title="Wavelength (nm)"),
            yaxis=dict(title="Transmission [dB]"),
        )
    ...
    if num == 4:
        ...
        device = ContraDC(
            polyfit_file=os.path.join(
                dir_path, "SiN_1550_TE_w1_850nm_w2_1150nm_thickness_400nm.txt"
            ),
            period=335e-9,
        )
        device.simulate()
    ...
    if num == 5:
        ...
        device = ContraDC(
            w1=w1, w2=w2, apod_shape=apod_shape, period=period, kappa=25000
        )
        device.simulate()
