    ...
    **Class Attributes**: Are calculated by the different member functions during simulation.
    ...
    def getPropConstants(self):
        ...
        if self.polyfit_file is None:
            ...
            n1 = np.reshape(
                np.loadtxt(os.path.join(filepath_ContraDC, "Database/neff/neff_1.txt")),
                (5, 5, 5),
            )
            n2 = np.reshape(
                np.loadtxt(os.path.join(filepath_ContraDC, "Database/neff/neff_2.txt")),
                (5, 5, 5),
            )
            w1_w2_wvl = np.loadtxt(
                os.path.join(filepath_ContraDC, "Database/neff/w1_w2_lambda.txt")
            )
            ...
            d = np.transpose(
                np.stack((w1_tiled, w2_tiled, wavelength_tiled)), (1, 2, 0)
            )
            ...
            with open(self.polyfit_file, "r") as f:
                ...
                wavelength_tiled = np.transpose(
                    np.tile(self.wavelength, (self.N_seg, 1))
                )
    ...
    def getApodProfile(self):
        ...
        if self.apod_shape == "gaussian":
            ...
            alpha, beta = 2, 3
            apod = (
                1 / 2 * (1 + np.tanh(beta * (1 - 2 * abs(2 * z / self.N_seg) ** alpha)))
            )
            apod = np.append(
                np.flip(apod[0 : int(apod.size / 2)]), apod[0 : int(apod.size / 2)]
            )
            apod *= self.kappa
    ...
    def getChirpProfile(self):
        ...
        if self.polyfit_file is None:
            ...
            if self.w1_profile is None:
                ...
                self.w1_profile = (
                    np.round(self.w1_profile / self.w_chirp_step) * self.w_chirp_step
                )
            ...
            if self.w2_profile is None:
                ...
                self.w2_profile = (
                    np.round(self.w2_profile / self.w_chirp_step) * self.w_chirp_step
                )
        ...
        if self.period_profile is None:
            ...
            self.period_profile = np.linspace(
                self.period[0], self.period[-1], self.N_seg
            )
    ...
    def propagate(self):
        ...
        S2[:, :, 3, 0] = (
            1j * np.conj(kappa_12) * np.exp(-1j * (beta_del_1 + beta_del_2) * l_cum)
        )
    ...
    def generate_dat(self, filepath="", make_plot=True):
        ...
        if make_plot:
            ...
            layout = go.Layout(
                title="Compact model passivity test",
            ...
            layout = go.Layout(
                title="Compact model reciprocity test",
                xaxis=dict(title="X Axis"),
                yaxis=dict(title="Y Axis"),
            )
        ...
        filename = (
            ...
            + str(self.N)
            + ",s="
            + str(1 if self.sinusoidal else 0)
            + ",a="
            + "%.2f" % self.a
            + ",rib="
            + str(1 if self.rib else 0)
            + ",pol="
            + str(0 if self.pol == "TE" else 1)
            + ",l1="
            + "%.0f" % (self.wvl_range[0] * 1e9)
            + ",l2="
            + "%.0f" % (self.wvl_range[1] * 1e9)
            + ",ln="
            + str(self.resolution)
            + ".dat"
        )
        ...
        with open(path, "w") as f:
            ...
            # Write S11 data
            f.write(
                f"('port 1',{mode_label},{mode_ID},'port 1',{mode_ID},'transmission')\n"
            )
            ...
            # Write S21 data
            f.write(
                f"('port 2',{mode_label},{mode_ID},'port 1',{mode_ID},'transmission')\n"
            )
            f.write(f"({FREQ_PTS},3)\n")
            ...
            # Write S31 data
            f.write(
                f"('port 3',{mode_label},{mode_ID},'port 1',{mode_ID},'transmission')\n"
            )
            f.write(f"({FREQ_PTS},3)\n")
            ...
            # Write S41 data
            f.write(
                f"('port 4',{mode_label},{mode_ID},'port 1',{mode_ID},'transmission')\n"
            )
            f.write(f"({FREQ_PTS},3)\n")
            ...
            # Write S12 data
            f.write(
                f"('port 1',{mode_label},{mode_ID},'port 2',{mode_ID},'transmission')\n"
            )
            f.write(f"({FREQ_PTS},3)\n")
            ...
            # Write S22 data
            f.write(
                f"('port 2',{mode_label},{mode_ID},'port 2',{mode_ID},'transmission')\n"
            )
            f.write(f"({FREQ_PTS},3)\n")
            ...
            # Write S32 data
            f.write(
                f"('port 3',{mode_label},{mode_ID},'port 2',{mode_ID},'transmission')\n"
            )
            f.write(f"({FREQ_PTS},3)\n")
            ...
            # Write S42 data
            f.write(
                f"('port 4',{mode_label},{mode_ID},'port 2',{mode_ID},'transmission')\n"
            )
            f.write(f"({FREQ_PTS},3)\n")
            ...
            # Write S13 data
            f.write(
                f"('port 1',{mode_label},{mode_ID},'port 3',{mode_ID},'transmission')\n"
            )
            f.write(f"({FREQ_PTS},3)\n")
            ...
            # Write S23 data
            f.write(
                f"('port 2',{mode_label},{mode_ID},'port 3',{mode_ID},'transmission')\n"
            )
            f.write(f"({FREQ_PTS},3)\n")
            ...
            # Write S33 data
            f.write(
                f"('port 3',{mode_label},{mode_ID},'port 3',{mode_ID},'transmission')\n"
            )
            f.write(f"({FREQ_PTS},3)\n")
            ...
            # Write S43 data
            f.write(
                f"('port 4',{mode_label},{mode_ID},'port 3',{mode_ID},'transmission')\n"
            )
            f.write(f"({FREQ_PTS},3)\n")
            ...
            # Write S14 data
            f.write(
                f"('port 1',{mode_label},{mode_ID},'port 4',{mode_ID},'transmission')\n"
            )
            f.write(f"({FREQ_PTS},3)\n")
            ...
            # Write S24 data
            f.write(
                f"('port 2',{mode_label},{mode_ID},'port 4',{mode_ID},'transmission')\n"
            )
            f.write(f"({FREQ_PTS},3)\n")
            ...
            # Write S34 data
            f.write(
                f"('port 3',{mode_label},{mode_ID},'port 4',{mode_ID},'transmission')\n"
            )
            f.write(f"({FREQ_PTS},3)\n")
            ...
            # Write S44 data
            f.write(
                f"('port 4',{mode_label},{mode_ID},'port 4',{mode_ID},'transmission')\n"
            )
            f.write(f"({FREQ_PTS},3)\n")
    ...
    def simulate_kappa(self):
        ...
        script = script.replace("W1 = 560e-9", "W1 = {:1.6g}".format(self.w1))
        script = script.replace("W2 = 440e-9", "W2 = {:1.6g}".format(self.w2))
        script = script.replace("dW1 = 48e-9", "dW1 = {:1.6g}".format(self.dw1))
        script = script.replace("dW2 = 24e-9", "dW2 = {:1.6g}".format(self.dw2))
        script = script.replace("gap = 100e-9", "gap = {:1.6g}".format(self.gap))
        script = script.replace(
            "period = 318e-9", "period = {:1.6g}".format(self.period)
        )
        script = script.replace("angle = 83", "angle = {:1.6g}".format(self.angle))
        script = script.replace("rib = false", "rib = " + str(self.rib).lower())
        script = script.replace(
            "thickness_device = 220e-9",
            "thickness_device = {:1.6g}".format(self.thickness_device),
        )
        script = script.replace(
            "thickness_rib = 90e-9",
            "thickness_rib = {:1.6g}".format(self.thickness_rib),
        )
        script = script.replace(
            "mat_device = 'Si (Silicon) - Dispersive & Lossless'",
            "mat_device = '" + self.mat_device + "'",
        )
        script = script.replace("pol = 'TE'", "pol = '" + self.pol.upper() + "'")
        script = script.replace(
            "wl_min = 1.5e-6", "wl_min = {:1.6g}".format(self.wvl_range[0])
        )
        script = script.replace(
            "wl_max = 1.6e-6", "wl_max = {:1.6g}".format(self.wvl_range[1])
        )
    ...
    def displayResults(self, tag_url=False):
        ...
        text_color = np.asarray([0, 0, 0]) + 0.25
        ...
        for i, item in zip(range(len(self.performance)), self.performance):
            ...
            plt.text(
                0.5,
                -i,
                str(self.performance[item][0]) + " " + self.performance[item][1],
                fontsize=11,
                ha="left",
                va="bottom",
                color=text_color,
        ...
        if tag_url:
            ...
            plt.text(
                self._wavelength.min(),
                min(self.drop.min(), self.thru.min()),
                url,
                va="top",
                color="grey",
                size=6,
