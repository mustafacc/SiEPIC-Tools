...
def layout_check(cell=None, verbose=False, GUI=False, timing=False, file_rdb=None):
    ...
    if timing:
        print(
            "*** layout_check(), timing; done nets (%s), components (%s) "
            % (len(nets), len(components))
        )
        print("    Time elapsed: %s" % (time() - time1))
    ...
    if not components:
        if Python_Env == "KLayout_GUI":
            v = pya.MessageBox.warning(
                "Errors",
                "No components found (using SiEPIC-Tools DevRec and PinRec definitions). Cannot perform Verification.",
                pya.MessageBox.Ok,
            )
            return
        else:
        ...
        for p in pin_errors:
            if p[0].polygon():
                ...
                rdb_item = rdb.create_item(
                    rdb_cell.rdb_id(), rdb_cat_id_comp_pinerrors.rdb_id()
                )
    ...
    if verification:
        ...
        try:
            deviceonly_layers = eval(
                verification["verification"]["shapes-inside-components"][
                    "deviceonly-layers"
                ]
            )
        except:
            deviceonly_layers = [[1, 0]]
        deviceonly_layers_ids = [
            ly.find_layer(*l)
            for l in deviceonly_layers
            if ly.find_layer(*l) is not None
        ]
        ...
        if verbose:
            ...
            while not iter1.at_end():
                ...
                iter1.next()
        iter1 = pya.RecursiveShapeIterator(ly, cell, deviceonly_layers_ids)
        iter1.unselect_cells(
            [c.cell_index() for c in cells_components if c is not None]
        )
        extra_shapes = []
        while not iter1.at_end():
        ...
        if verbose:
            print(
                " - found %s shape(s) not belonging to components " % len(extra_shapes)
            )
        ...
        for e in extra_shapes:
            if e[0].dpolygon:
                rdb_item = rdb.create_item(
                    rdb_cell.rdb_id(), rdb_cat_id_comp_shapesoutside.rdb_id()
                )
                rdb_item.add_value(
                    pya.RdbItemValue(
                        e[0].dpolygon.transformed(e[1].to_trans().to_itrans(dbu))
                    )
                )
    ...
    print(
        "DevRec Regions: original %s, merged %s"
        % (region.count(), region.merge().count())
    )
    """
    ...
    if rdb.num_items() > 0:
        ...
        if Python_Env == "KLayout_GUI":
            v = pya.MessageBox.warning("Errors", msg, pya.MessageBox.Ok)
