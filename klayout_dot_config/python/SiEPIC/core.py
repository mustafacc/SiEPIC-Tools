...
class Net:
    def __init__(self, idx=None, _type=None, pins=None):
        """
        Net:
        Connectivity between pins. Used for netlist generation and verification.
        ...
        Args:
            idx (int, optional): net number, index, should be unique, 0, 1, 2, ...
            _type (str, optional): one of PIN_TYPES, defined in SiEPIC._globals.PINTYPES
            pins (list, optional): pin array, Pin[]. for backwards linking (optional)
        """
        self.idx = idx
        self.type = _type
        self.pins = pins
    ...
    def display(self):
        log_message = f"- net: {self.idx}, pins: {[ [p.pin_name, p.center.to_s(), p.component.component, p.component.instance] for p in self.pins ]}"
        # Log the message at the desired log level (e.g., INFO)
...
class Pin:
    def __init__(
        ...
        Args:
            path (_type_, optional): the pin's Path (Optical)
            _type (str, optional): one of PIN_TYPES, defined in SiEPIC._globals.PINTYPES
            box (_type_, optional): the pin's Box (Electrical)
            polygon (_type_, optional): the pin's polygon (Optical IO)
            component (_type_, optional): which component index this pin belongs to
            net (_type_, optional): _description_. Defaults to None.
            pin_name (str, optional): label read from the cell layout (PinRec text)
        """
        ...
        if path:
            ...
            if len(pts) == 2:
                ...
                logging.info(
                    f"class Pin():__init__: detected invalid Pin, number of points: {len(pts)}"
                )
                self.rotation = 0
                return
            self.rotation = angle_vector(
                pts[1] - pts[0]
        ...
        if polygon:
            ...
            self.center = (
                polygon.bbox().center()
            )  # center of the pin: a Point (relative coordinates, within component)
    ...
    def display(self):
        p = self
        logging.info(
            f"- pin_name {p.pin_name}: component_idx {p.component.idx}, pin_type {p.type}, rotation: {p.rotation}, net: {p.net.idx}, ({p.center}), path: {p.path}"
        )
        o = self


def display_pins(pins):
    logging.info("Displaying Pins:")
    for o in pins:
        o.display()
...
class Component:
    def __init__(
        ...
        Args:
            idx (int, optional): component index, should be unique, 0, 1, 2, ...
            component (_type_, optional): which component (name) this belongs to
            instance (_type_, optional): which component (instance) this belongs to. TODO: Needs to fixed to be pya.Instance
            trans (pya.trans, optional): instance's location (.disp.x, y), mirror (.is_mirror), rotation (angle);
            library (_type_, optional): compact model library
            params (_type_, optional): # Spice parameters
            waveguide_type (str, optional): Waveguide Type, referring to Waveguides.XML
            pins (list, optional): an array of all the optical pins, Pin[]
            epins (list, optional): _description_. Defaults to [].
            nets (list, optional): _description_. Defaults to [].
            polygon (_type_, optional): _description_. Defaults to None.
            DevRec_polygon (_type_, optional): The component's DevRec polygon/box outline (absolute coordinates, i.e., transformed)
            cell (pya.Cell, optional): component's cell
            basic_name (_type_, optional): component's basic_name (especially for PCells)
            cellName (str, optional): component's Library Cell name
        """
        self.idx = idx
        self.component = component
    ...
    def display(self):
        ...
        for c in cc:
            c.npins = len(c.pins)
            text = (
                f"- basic_name: {c.basic_name}, component: {c.component}-{c.idx} / {c.instance}; transformation: {c.trans}; center position: {c.Dcenter}; number of pins: {c.npins}; optical pins: {[ [p.pin_name, p.center.to_s(), p.net.idx] for p in c.pins if p.type == _globals.PIN_TYPES.OPTICAL ]}; electrical pins: {[ [p.pin_name, p.center.to_s(), p.net.idx] for p in c.pins if p.type == _globals.PIN_TYPES.ELECTRICAL ]}; optical IO pins: {[ [p.pin_name, p.center.to_s(), p.net.idx] for p in c.pins if p.type == _globals.PIN_TYPES.OPTICALIO ]}; has compact model: {c.has_model()}; params: {c.params}."
            )
            logging.info(text)
        return text
        ...
        for s in dicta:
            ...
            if "." not in s.split("=")[1] and "e" not in s.split("=")[1].lower():
                ...
                if "[" in string:
                    ...
                    string = (
                        string.replace("u", "e-6").replace("n", "e-9").replace(";", "")
                    )
                    # print (string)
    ...
    def set_SPICE_params(self, arg, verbose=False):
        ...
        while not (iter_sh.at_end()):  # Find cell where SPICE params are stored
            if iter_sh.shape().is_text():
                ...
                if text.string.find("Spice_param:") > -1:
                    new_text = pya.Text(
                        newSPICE_text, shape.text_trans, shape.text_size, -1
                    )
        ...
        if cell._is_const_object():
            cell_inst = cell.layout().cell(
                cell_idx
            )  # Need to do this to avoid error (See KLayout issue #235)
        else:
            cell_inst = cell

        t = pya.Trans(
            pya.Trans.R0, pya.Point(0, 0)
        )  # Coordinates are with respect to the cell center
        new_text = pya.Text(newSPICE_text, t, 0.1 / ly.dbu, -1)
        cell_inst.shapes(LayerDevRecN).insert(new_text)
        self.params = spice_str
        return True

    def get_SPICE_params(
        self,
    ):  # Retturns a SPICE parameter string (without the 'Spice_param:' label)
        return self.params
    ...
    def has_model(self):
        ...
        if self.library and self.component:
            return (
                self.library.lower().replace("/", "::") + "::" + self.component.lower()
            ) in INTC_ELEMENTS
        else:
            ...
            while not (s.at_end()):
                if s.shape().is_path():
                    ...
                    pts[1] = (
                        pts[1]
                        - pya.Point(
                            int(math.cos(rotation) * 1000),
                            int(math.sin(rotation) * 1000),
                        )
                    ).to_p()
...
class WaveguideGUI:
    ...
    def __init__(self):
        ...
        ui_file = pya.QFile(
            os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "files", "waveguide_gui.ui"
            )
        )
        ui_file.open(pya.QIODevice().ReadOnly)
        self.window = pya.QFormBuilder().load(
            ui_file, pya.Application.instance().main_window()
        )
        ui_file.close
    ...
    def update(self):
        ...
        if 0:
            ...
            waveguide_types_simple = [
                t for t in waveguide_types if not "compound_waveguide" in t.keys()
            ]
        ...
        try:
            ...
            raise Exception(
                "No waveguides found for technology=%s. Check that there exists a technology definition file %s.lyt and a WAVEGUIDES.xml file in the PDK folder. \n(Error in SiEPIC.core.WaveguideGUI.update)"
                % (tech_name, tech_name)
            )
    ...
    def get_parameters(self, show):
        ...
        params = {
            "radius": float(self.window.findChild("radius").text),
        ...
        if not self.window.findChild("configuration").currentText == "":
            waveguide = [wg for wg in self.waveguides if wg["name"] == waveguide_type][
                0
            ]
            params["waveguide_type"] = self.window.findChild(
                "configuration"
            ).currentText
            ...
            if "component" in waveguide.keys():
                ...
                if "crossing" in waveguide:
                    ...
                    if "crossing_library" in waveguide["crossing"]:
                        params["crossing_library"] = waveguide["crossing"][
                            "crossing_library"
                        ]
                    else:
...
class MonteCarloGUI:
    ...
    def tech_changed(self, val):
        options = [
            t
            for t in self.montecarlos
            if t["name"] == self.window.findChild("technology").currentText
        ]
        if options:
            technology = options[0]
            self.window.findChild("std_dev").text = technology["wafer"]["width"][
                "std_dev"
            ]
            self.window.findChild("corr_len").text = technology["wafer"]["width"][
                "corr_length"
            ]
            self.window.findChild("std_dev_2").text = technology["wafer"]["height"][
                "std_dev"
            ]
            self.window.findChild("corr_len_2").text = technology["wafer"]["height"][
                "corr_length"
            ]
            self.window.findChild("std_dev_3").text = technology["wafer_to_wafer"][
                "width"
            ]["std_dev"]
            self.window.findChild("std_dev_4").text = technology["wafer_to_wafer"][
                "thickness"
            ]["std_dev"]
    ...
    def get_parameters(self):
        ...
        return {
            ...
            "waf_to_waf_var": {
                "width": {"std_dev": float(self.window.findChild("std_dev_3").text)},
                "thickness": {
