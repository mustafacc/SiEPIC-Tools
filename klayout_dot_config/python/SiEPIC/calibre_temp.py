def calibreDRC(params=None, cell=None):
    ...
    if not CALIBRE:
        pya.MessageBox.warning(
            "Missing CALIBRE.xml",
            "Missing CALIBRE.xml. This configuration file should be located in the tech folder for the active technology.",
            pya.MessageBox.Ok,
        )
        return
    ...
    if 1:
        ...
        if "Darwin" in platform.version():
            ...
            cmd = '/usr/bin/ssh %s "%s"' % (
                ssh_server,
                "cd " + remote_path + "; source run_calibre;",
            )
            print(cmd)
            out = commands.getstatusoutput(cmd)
            print(out)
            ...
            cmd = "/usr/bin/scp %s:%s %s" % (
                ssh_server,
                remote_path + "/drc.rve",
                pipes.quote(results_pathfile),
            )
            print(cmd)
            out = commands.getstatusoutput(cmd)
            print(out)
        ...
        if version.find("2.") > -1:
            ...
            out = cmd(
                'cd "%s" && scp -i C:/Users/bpoul/.ssh/drc -P%s "%s" %s:%s'
                % (
                    os.path.dirname(local_pathfile),
                    params["port"],
                    os.path.basename(local_pathfile),
                    params["url"],
                    remote_path,
            ...
            out = cmd(
                'cd "%s" && scp -i C:/Users/bpoul/.ssh/drc -P%s %s %s:%s'
                % (local_path, params["port"], "drc.cal", params["url"], remote_path)
            )
            ...
            out = cmd(
                'ssh -i C:/Users/bpoul/.ssh/drc -p%s %s "%s"'
                % (
                    params["port"],
                    params["url"],
                    "cd " + remote_path + " && source run_calibre",
        ...
        if os.path.exists(results_pathfile):
            ...
            pya.MessageBox.warning(
                "Errors",
                "Something failed during the server Calibre DRC check.",
                pya.MessageBox.Ok,
            )
