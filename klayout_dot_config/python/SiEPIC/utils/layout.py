...
def layout_waveguide4(cell, dpath, waveguide_type, debug=False):
    ...
    if type(params) == type([]) and len(params) > 0:
        ...
        raise Exception(
            "error: waveguide type (%s) not found in PDK waveguides" % waveguide_type
        )
    ...
    if "compound_waveguide" in params:
        ...
        # find the taper
        if (
            "taper_library" in params["compound_waveguide"]
            and "taper_cell" in params["compound_waveguide"]
        ):
            ...
            if distance_points < min_length:
                ...
                if ii == len(pts) - 1:
                    ...
                    waveguide_length += layout_waveguide3(
                        subcell, wg_sm_segment_pts, params_singlemode, debug=False
                    )
            else:
                ...
                if round(angle) % 360 == 270.0:
                    t = Trans(
                        Trans.R270, start_point.x, start_point.y - offset + wg_first
                    )
                    t2 = Trans(Trans.R90, end_point.x, end_point.y + offset - wg_last)
                    wg_start_pt = Point(
                        start_point.x, start_point.y - offset - taper_length + wg_first
                    )
                    wg_end_pt = Point(
                        end_point.x, end_point.y + offset + taper_length - wg_last
                    )
                if round(angle) % 360 == 90.0:
                    t = Trans(
                        Trans.R90, start_point.x, start_point.y + offset - wg_first
                    )
                ...
                if round(angle) % 360 == 180.0:
                    t = Trans(
                        Trans.R180, start_point.x - offset + wg_first, start_point.y
                    )
                    t2 = Trans(Trans.R0, end_point.x + offset - wg_last, end_point.y)
                    wg_start_pt = Point(
                        start_point.x - offset - taper_length + wg_first, start_point.y
                    )
                    wg_end_pt = Point(
                        end_point.x + offset + taper_length - wg_last, end_point.y
                    )
                if round(angle) % 360 == 0.0:
                    t = Trans(
                        Trans.R0, start_point.x + offset - wg_first, start_point.y
                    )
                ...
                cell.insert(CellInstArray(subcell.cell_index(), Trans()))
                waveguide_length += layout_waveguide3(
                    subcell, [wg_start_pt, wg_end_pt], params_multimode, debug=False
                )
                # compound segment
                if ii > 1:
...
def layout_waveguide3(cell, pts, params, debug=False):
    ...
    # draw the waveguide
    sbends = (
        params["sbends"].lower() in ["true", "1", "t", "y", "yes"]
        if "sbends" in params.keys()
        else False
    )
    waveguide_length = layout_waveguide2(
        ...
        layout,
        cell,
        [wg["layer"] for wg in params["component"]],
    ...
    text = Text(
        "Lumerical_INTERCONNECT_library=Design kits/%s" % CML, t, 0.1 * wg_width, -1
    )
    text.halign = halign
    ...
    pts_txt = str(
        [[round(p.to_dtype(dbu).x, 3), round(p.to_dtype(dbu).y, 3)] for p in pts]
    ).replace(", ", ",")
    text = Text(
        'Spice_param:wg_length=%.9f wg_width=%.3g points="%s" radius=%.3g'
        % (waveguide_length * 1e-6, wg_width * 1e-9, pts_txt, radius * 1e-6),
        t,
        0.1 * wg_width,
        -1,
    )
    ...
    text = Text("Length=%.3f (microns)" % (waveguide_length), t, 0.5 * wg_width, -1)
    text.halign = halign
    shape = cell.shapes(LayerDevRecN).insert(text)
...
def layout_waveguide2(
    ...
    for lr in range(0, len(layers)):
        ...
        for i in it:
            turn = (
                (angle_b_vectors(pts[i] - pts[i - 1], pts[i + 1] - pts[i]) + 90) % 360
                - 90
            ) / 90
            ...
            if (sbends) and i < len(pts) - 2:
                ...
                if angle == angle2 and dis2 < 2 * pt_radius:  # An SBend may be inserted
                    ...
                    if (i < 3 and dis1 < curved_l / 2) or (
                        i > len(pts) - 4 and dis3 < curved_l / 2
                    ):
                        pass  # Check if there is partial clearance for the bend when there is an end near
                    elif (i >= 3 and (dis1 - pt_radius < curved_l / 2)) or (
                        i <= len(pts) - 4 and (dis3 - pt_radius < curved_l / 2)
                    ):
                        ...
                        if not (angle % 2):
                            t = pya.Trans(
                                angle,
                                (angle == 2),
                                pts[i].x + (angle - 1) * int(curved_l / 2),
                                pts[i].y,
                            )
                        else:
                            t = pya.Trans(
                                ...
                                (angle == 1),
                                pts[i].x,
                                pts[i].y - (angle) * int(curved_l / 2),
                            )
                        bend_pts = (
                            pya.DPath(
                                bezier_parallel(
                                    pya.DPoint(0, 0),
                                    pya.DPoint(curved_l * dbu, h * dbu),
                                    0,
                                ),
                                0,
                            )
                        ...
                        cell.shapes(layout.layer(TECHNOLOGY["Text"])).insert(
                            text
                        ).text_size = (3 / dbu)
                        # Mark the start of the SBend with an "s"
        ...
        wg_polygon = Polygon(
            translate_from_normal(wg_pts, width / 2 + (offset if turn > 0 else -offset))
            + translate_from_normal(
...
def layout_arc(cell, layer, center, r, w, theta_start, theta_end, ex=None):
    ...
    # optimal sampling
    def arc_function(t):
        return np.array([center.x + r * np.cos(t), center.y + r * np.sin(t)])
...
def layout_arc_drc_exclude(
    cell, drc_layer, center, r, w, theta_start, theta_end, ex=None
):
    corner_points = [
...
def layout_arc_with_drc_exclude(
    cell, layer, drc_layer, center, r, w, theta_start, theta_end, ex=None
):
...
def layout_waveguide_sbend_bezier(
    cell, layer, trans, w=0.5, wo=None, h=2.0, length=15.0, insert=True, debug=False
):
...
def layout_waveguide_sbend(
    cell, layer, trans, w=500, r=25000, h=2000, length=15000, insert=True, dbu=0.001
):
    """Lays out an s-bend
    ...
    if straight_l >= 0:
        ...
        npoints = int(
            points_per_circle(r * cell.layout().dbu, dbu=dbu) * circle_fraction
        )
        if npoints == 0:
            npoints = 1
...
def place_cell(parent_cell, cell, placement_origin, params=None, relative_to=None):
    ...
    if relative_to is not None:
        offset = next(
            (port.position for port in ports if port.name == relative_to), None
        )
    ...
    for i in range(0, tree_depth):
        ...
        for j in range(0, int(2 ** (tree_depth - i - 1))):
            ...
            if (i > 0) and draw_waveguides:
                ...
                connect_pins_with_waveguide(
                    inst[j], "opt3", inst_higher[j * 2], "opt1", waveguide_type=wg_type
                )
            y += dy
        inst_higher = inst
        if i == 0:
