...
def on_segment(segment, p, off=(0, 0)):
    ...
    )  # Rounding is needed when the paths are not snaped to the grid
    ...
    for i in range(path1.num_points() - 1):
        ...
        for j in range(path2.num_points() - 1):
            ...
            if x_center:
                ...
                if on_segment((p1_0, p1_1), x_center, off_x) and on_segment(
                    (p2_0, p2_1), x_center, off_y
                ):
...
def new_OIP(
    points, layer=0, _cell=0, cv_index=0
):  # uses obj as a reference for all the new instantiation
...
def breakPath(path, list_inter, crossing_bbox, obj, verbose=False):
    ...
    for i in range(path.num_points() - 1):
        ...
        for center in new_list_inter:
            if on_segment((p0, p1), center):
                ...
                new_list.append(
                    new_OIP(
                        newPath, layer=obj.layer, _cell=obj.top, cv_index=obj.cv_index
                    )
                )
    ...
    new_list.append(
        new_OIP(
            newPath - old_bbox, layer=obj.layer, _cell=obj.top, cv_index=obj.cv_index
        )
    )
    if verbose:
        print("Appended path: ", new_list[-1].shape.path.to_s())
...
def cross_2paths(
    oip_path1, oip_path2, xcell, offset=0, origin=pya.Trans(0, 0), verbose=False
):
    ...
    if verbose:
        print(
            "Found %s crossings between paths %s and %s"
            % (len(list_inter), oip_path1, oip_path2)
        )
...
def insert_crossing(selected_paths, params=None, verbose=False):
    ...
    if "crossing_cell" in params.keys():
        if "crossing_library" in params.keys():
            xcell = top_cell.layout().create_cell(
                params["crossing_cell"], params["crossing_library"]
            )
        else:
            xcell = top_cell.layout().create_cell(
                params["crossing_cell"], TECHNOLOGY["technology_name"]
            )
    if xcell == None:
        if verbose:
            print(
                "SiEPIC.utils.insert_crossing(): No available crossing cell in technology ",
    ...
    for obj1 in iter_paths1:
        ...
        for obj2 in iter_paths2:
            ...
            new_paths1, new_paths2, check = cross_2paths(
                obj1,
                obj2,
                xcell,
                offset=int(params["radius"] / dbu),
                origin=pya.Trans(
                    int(crossing_origin[0] / dbu), int(crossing_origin[1] / dbu)
                ),
                verbose=verbose,
            )
            ...
            if check:
                ...
                iter_paths1.insert(
                    idx_obj2, new_paths2
                )  # insert the split parts of the path

                iter_paths1.pop_and_del(idx_obj1)  # then remove the first oip
                iter_paths1.insert(
                    idx_obj1, new_paths1
                )  # and insert the split parts of the path
                # iter_paths1.set_index(-1);
                iter_paths1.prev()  # Repeat the comaprison from the first of the newly split paths

                test_instances = (
                    iter_paths1.get_data()
                )  # Update the outer iterator data to onclude the new paths in the exploration
                break

    end = time.time()
    if verbose:
        print(
            "SiEPIC.utils.insert_crossing(): Finished finding crossing, ellapsed time: %s s"
            % (end - start)
        )
...
def example(
    ...
    if params is None:
        if verbose:
            print(
                "SiEPIC.utils.insert_crossing() EXAMPLE: No parameters returned (user pressed Cancel); returning..."
            )
        return
    if verbose:
    ...
    global SiEPIC_crossings_UI_insertcrossing_flag
    SiEPIC_crossings_UI_insertcrossing_flag = (
        True  # Flag to show the message to make crossings
    )
