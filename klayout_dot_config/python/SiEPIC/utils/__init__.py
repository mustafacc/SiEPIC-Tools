...
def get_library_names(tech_name, verbose=False):
    ...
    if (
        KLAYOUT_VERSION < 27
    ):  #  technologies in 0.27: https://www.klayout.de/doc-qt5/code/class_Library.html#method24
    ...
    if techname not in sys.modules:
        ...
        if path_lyt_file:
            ...
            if matches:
                ...
                raise Exception(
                    "SiEPIC.load_klayout_technology: could not load KLayout technology %s within folder %s"
                    % (techname, path_module)
                )
...
def get_technology_by_name(tech_name, verbose=False):
    ...
    if not tech_name:
        raise Exception(
            ...
            "Problem with active Technology: please activate a technology (not Default)",
            pya.MessageBox.Ok,
        )
        ...
        for file in os.listdir(os.path.join(technology["base_path"], "cml")):
            if file.lower().endswith(".cml"):
                ...
                if not cml_name in cml_names:
                    ...
                    if ("v" + cml_version) > cml_versions[cml_ind]:
                        cml_files[cml_ind] = file
                        cml_paths[cml_ind] = os.path.join(
                            technology["base_path"], "cml", file
                        )
                        cml_versions[cml_ind] = "v" + cml_version
    ...
    if "layer-properties-tabs" in xml_dict:
        ...
        layer_dict = xml_dict["layer-properties-tabs"]["layer-properties"][
            lv.current_layer_list
        ]["properties"]
    else:
    ...
    def get_members(layer_dict, technology):
        if isinstance(layer_dict, list):
            ...
            if layer_dict["source"] != "*/*@*":
                technology[layer_dict["name"]] = pya.LayerInfo(
...
def get_technology(verbose=False, query_activecellview_technology=False):
    ...
    if _globals.Python_Env != "KLayout_GUI":
        raise Exception(
            "SiEPIC.utils.get_technology: this function can only be used in KLayout Application graphical interface mode."
        )
    else:
        try:
            lv = pya.Application.instance().main_window().current_view()
...
def load_Waveguides_by_Tech(tech_name, debug=False):
    ...
    for root, dirnames, filenames in os.walk(folder, followlinks=True):
        [
            ...
            if fnmatch.filter(filenames, tech_name + ".lyt")
        ]
        [
            paths.append(os.path.join(root, filename))
    ...
    # and find WAVEGUIDE.xml and WAVEGUIDE_*.xml files
    libs = [
        pya.Library.library_by_id(lib)
        for lib in pya.Library.library_ids()
        if tech_name in pya.Library.library_by_id(lib).technologies()
    ]
            ...
            if debug:
                print(
                    ...
                    % (
                        ...
                        filenames,
                        (
                            fnmatch.filter(filenames, "WAVEGUIDES.xml")
                            + fnmatch.filter(filenames, "WAVEGUIDES_*.xml")
                        ),
                    )
                )
            [
    ...
    if not (waveguides):
        print(
            "No waveguides found for technology=%s. Check that there exists a technology definition file %s.lyt and WAVEGUIDES.xml file"
            % (tech_name, tech_name)
        )
...
def load_DFT(TECHNOLOGY=None, debug=True):
    ...
    if Python_Env == "KLayout_GUI":
        ...
        local_DFT_path = os.path.join(
            os.path.dirname(os.path.realpath(layout_filename)), filename + "_DFT.xml"
        )
...
def selected_opt_in_text():
    ...
    selection = [
        ...
        and o.shape.is_text()
        and "opt_in" in o.shape.text.string
    ]
    return selection
...
def select_paths(layer, cell=None, verbose=None):
    ...
    if selection == []:
        ...
        lv.object_selection = [
            o for o in selection if (not o.is_cell_inst()) and o.shape.is_path()
        ]
    if verbose:
...
def select_waveguides(cell=None):
    ...
    if selection == []:
        ...
        lv.object_selection = [
            o
            for o in selection
            if o.is_cell_inst() and o.inst().cell.basic_name() == "Waveguide"
        ]
...
def points_per_circle(radius, dbu=0.001):
    ...
    return (
        int(ceil(pi / acos(1 - err / radius))) if radius > 1 else 10
    )  # Lukas' derivation (same answer as below)
    ...
    for i in range(0, npoints + 1):
        pts.append(
            pya.Point.from_dpoint(
                pya.DPoint((r * cos(i * da + th)) / 1, (r * sin(i * da + th)) / 1)
            )
        )
    return pts
...
def arc_bezier(radius, start, stop, bezier, DevRec=None, dbu=0.001):
    ...
    diff = 1.0 / (N - 1)  # convert int to float
    xp = [0, (1 - bezier) * L, L, L]
    yp = [0, 0, bezier * L, L]
    ...
    for i in range(1, N - 1):
        ...
        pts.append(
            pya.Point(-L, 0)
            + pya.Point(
                t**3 * xA + t**2 * xB + t * xC + xD,
                t**3 * yA + t**2 * yB + t * yC + yD,
            )
        )
...
def arc_to_waveguide(pts, width):
    ...
    return pya.Polygon(
        translate_from_normal(pts, -width / 2.0)
        + translate_from_normal(pts, width / 2.0)[::-1]
    )
...
def pt_intersects_segment(a, b, c):
    ...
    if dot < 0:
        return False
    return (
        False if dot > (b.x - a.x) * (b.x - a.x) + (b.y - a.y) * (b.y - a.y) else True
    )
...
def layout_pgtext(cell, layer, x, y, text, mag, inv=False):
    ...
    pcell = cell.layout().create_cell(
        "TEXT", "Basic", {"text": text, "layer": layer, "mag": mag, "inverse": inv}
    )
    dbu = cell.layout().dbu
    cell.insert(
        pya.CellInstArray(pcell.cell_index(), pya.Trans(pya.Trans.R0, x / dbu, y / dbu))
    )


def find_automated_measurement_labels(
    topcell=None, LayerTextN=None, TECHNOLOGY=None, GUI=False
):
    ...
    text_out = (
        "% X-coord, Y-coord, Polarization, wavelength, type, deviceID, params <br>"
    )
            ...
            if text.string.find("opt") > -1:
                ...
                if GUI == True:
                    if fields[5] in device_ids and not duplicate:
                        ...
                        windowlabel.setText(
                            "Duplicate device-ids detected. Please make sure all device-ids are unique"
                        )
...
def svg_from_component(component, filename, verbose=False):
    ...
    x, y = (
        component.DevRec_polygon.bbox().center().x,
        component.DevRec_polygon.bbox().center().y,
    )
    print("x,y: %s, %s" % (x, y))
    width, height = (
    ...
    polygons_vertices = [
        [
            [
                round((vertex.x - x) * 100.0 / scale + s1 / x1, 2),
                round((y - vertex.y) * 100.0 / scale + s2 / y1, 2),
            ]
    ...
    try:  # not sure why the first time it gives an error (Windows 8.1 lukas VM), Mustafa: svgwrite is not a module available in KL windows python
        ...
        dwg = svgwrite.Drawing(
            filename, size=(str(s1) + "%", str(s2) + "%"), debug=False
        )
    except:
        pass
    try:
    ...
    for i in range(0, len(polygons_vertices)):
        ...
        p = dwg.add(
            dwg.polyline(polygons_vertices[i], fill=color, debug=False)
        )  # stroke=color
