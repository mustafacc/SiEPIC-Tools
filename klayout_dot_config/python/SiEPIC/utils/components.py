...
def get_deviceonly_layers(cell, verbose=False):
    ...
    deviceonly_layers_ids = [
        ly.find_layer(*l) for l in deviceonly_layers if ly.find_layer(*l) is not None
    ]
...
if __name__ == "__main__":
    ...
    if len(o_selection) != 1:
        v = pya.MessageBox.warning(
            "No selection",
            "Select one cell you wish to have turned into a SiEPIC Component, with the DevRec and PinRec layers added.\nRead more about component requirements: https://github.com/SiEPIC/SiEPIC-Tools/wiki/Component-and-PCell-Layout.",
            pya.MessageBox.Ok,
        )
    else:
        ...
        if o_selection.is_cell_inst() == False:
            v = pya.MessageBox.warning(
                "No selection",
                "The selected object must be an instance of a cell (not primitive polygons)",
                pya.MessageBox.Ok,
            )
        else:
...
if 0:
    ...
    connect_pins_with_waveguide(
        ...
        turtle_B=[10, 90, 10, 90],
        turtle_A=[10, -90, 10, -90],
    )
    connect_pins_with_waveguide(
        inst,
        "opt2",
