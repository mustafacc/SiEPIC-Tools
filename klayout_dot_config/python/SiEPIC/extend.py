...
def cell_character_replacement(
    self, forbidden_cell_characters="=", replacement_cell_character="_"
):
...
def to_itype(self, dbu):
    path = pya.Path(
        [pt.to_itype(dbu) for pt in self.each_point()], round(self.width / dbu)
    )
...
def is_manhattan(self):
    ...
    for i in range(1, len(pts) - 1):
        if (
            (inner_angle_b_vectors(pts[i] - pts[i - 1], pts[i + 1] - pts[i]) + 90) % 360
            - 90
        ) != 90:
            return False
...
def radius_check(self, radius, dbu=0.001):
    ...
    if len(points) > 2:
        lengths = [
            points[i].distance(points[i - 1]) for i, pt in enumerate(points) if i > 0
        ]
...
def remove_colinear_points(self, verbose=False):
    ...
    for i in range(1, len(pts) - 1):
        turn = (
            (angle_b_vectors(pts[i] - pts[i - 1], pts[i + 1] - pts[i]) + 90) % 360 - 90
        ) / 90
    ...
    self.points = (
        ...
        + [
            pts[i]
            for i in range(1, len(pts) - 1)
            if not pt_intersects_segment(pts[i + 1], pts[i - 1], pts[i])
        ]
        + [pts[-1]]
    )
    return self
...
def translate_from_center(self, offset):
    ...
    for i in range(0, len(pts)):
        ...
        if offset < 0:
            o1 = pya.DPoint(
                abs(offset) * cos(angle_vector(u) * pi / 180 - pi / 2),
                abs(offset) * sin(angle_vector(u) * pi / 180 - pi / 2),
            )
            o2 = pya.DPoint(
                abs(offset) * cos(angle_vector(v) * pi / 180 + pi / 2),
    ...
    def snap_endpoints(input_pts, pins):
        ...
        pins_sorted = sorted(
            [
                pin
                for pin in pins
                if round((ang - pin.rotation) % 360) == 180
                and pin.type == _globals.PIN_TYPES.OPTICAL
            ],
            key=lambda x: x.center.distance(pts[0]),
        )
        ...
        pins_sorted = sorted(
            [
                pin
                for pin in pins
                if round((ang2 - pin.rotation) % 360) == 180
                and pin.type == _globals.PIN_TYPES.OPTICAL
            ],
            key=lambda x: x.center.distance(pts[-1]),
        )
        if len(pins_sorted):
    ...
    def snap_endpoints(input_pts, pins):
        ...
        pins_sorted = sorted(
            [
                ...
                for pin in pins
                if round((ang - pin.rotation) % 360) == 180
                and pin.type == _globals.PIN_TYPES.ELECTRICAL
            ],
            key=lambda x: x.center.distance(pts[0]),
        )
...
pya.Path.snap = snap
pya.Path.snap_m = (
    snap_m  # function added by Karl McNulty (RIT) for metal pin functionality
)
...
def print_parameter_list(self):
    ...
    for p in self.get_parameters():
        if ~p.readonly:
            print(
                "Name: %s, %s, unit: %s, default: %s, description: %s%s"
                % (
                    p.name,
                    types[p.type],
                    p.unit,
...
def find_pins(self, verbose=False, polygon_devrec=None, GUI=False):
    ...
    if not "PinRec" in TECHNOLOGY:
        if GUI:
            pya.MessageBox.warning(
                ...
                "Problem with active Technology %s: missing layer PinRec"
    ...
    while not (it.at_end()):
            ...
            while not (iter2.at_end()):
                ...
                if pin_name and pin_path.num_points() == 2:
                    ...
                    pins.append(
                        Pin(
                            path=pin_path,
                            _type=_globals.PIN_TYPES.OPTICAL,
                            pin_name=pin_name,
                        )
                    )
            if pin_name == None or pin_path.num_points() != 2:
                print(
                    "Invalid pin Path detected: %s. Cell: %s" % (pin_path, subcell.name)
                )
                error_text += (
                    "Invalid pin Path detected: %s, in Cell: %s, Optical Pins must have a pin name.\n"
        ...
        if it.shape().is_simple_polygon() or it.shape().is_box():
            ...
            if pin_name == None:
                error_text += (
                    "Invalid pin Box detected: %s, Cell: %s, Electrical Pins must have a pin name.\n"
                    % (pin_box, subcell.name)
                )
            #        raise Exception("Invalid pin Box detected: %s.\nElectrical Pins must have a pin name." % pin_box)
            pins.append(
                Pin(box=pin_box, _type=_globals.PIN_TYPES.ELECTRICAL, pin_name=pin_name)
            )
    ...
    if not "FbrTgt" in TECHNOLOGY:
        if GUI:
            pya.MessageBox.warning(
                "Problem with Technology",
                "Problem with active Technology %s: missing layer FbrTgt"
                % (TECHNOLOGY["technology_name"]),
                pya.MessageBox.Ok,
            )
            return
        ...
        if it.shape().is_polygon():
            ...
            if not (
                [
                    ...
                    and p.polygon == it.shape().polygon.transformed(it.itrans())
                ]
            ):
                pins.append(
                    Pin(
                        polygon=it.shape().polygon.transformed(it.itrans()),
    ...
    if not "PinRecM" in TECHNOLOGY:
        if GUI:
            pya.MessageBox.warning(
                "Problem with Technology",
                "Problem with active Technology %s: missing layer PinRecM"
                % (TECHNOLOGY["technology_name"]),
                pya.MessageBox.Ok,
            )
            return
        ...
        if it.shape().is_path():
            ...
            # raise Exception("Invalid pin Path detected: %s, in Cell: %s.\nElectrical Pins must have a pin name." % (pin_path, subcell.name))
                ...
                pins.append(
                    Pin(
                        path=pin_path,
                        _type=_globals.PIN_TYPES.ELECTRICAL,
                        pin_name=pin_name,
                    )
                )
        it.next()
...
def find_components(self, cell_selected=None, inst=None, verbose=False):
    ...
    while not (iter1.at_end()):
        ...
        if found_component:
            ...
            if num_devrec > 1:
                print(
                    " * Warning: cell (%s) contains multiple (%s) DevRec layers, suggesting that a cell contains subcells with DevRec layers."
                    % (instance, num_devrec)
                )
            ...
            if 1:
                ...
                while not (iter2.at_end()):
                    if iter2.shape().is_text():
                        ...
                        if text.string.find("Lumerical_INTERCONNECT_library=") > -1:
                            library = text.string[
                                len("Lumerical_INTERCONNECT_library=") :
                            ]
                        if text.string.find("Lumerical_INTERCONNECT_component=") > -1:
                            component = text.string[
                                len("Lumerical_INTERCONNECT_component=") :
                            ]
                ...
                if library == None:
                    if verbose:
                        print(
                            "Missing library information for component: %s" % component
                        )
                if cellName == None:
                ...
                components.append(
                    Component(
                        ...
                        params=spice_params,
                        polygon=polygon,
                        DevRec_polygon=DevRec_polygon,
                        cell=subcell,
                        basic_name=subcell.basic_name(),
                        cellName=cellName,
                ...
                pins = sorted(
                    subcell.find_pins_component(components[-1]),
                    key=lambda p: "" if p.pin_name == None else p.pin_name,
                )
        ...
        if inst and components:
            if (
                components[-1].basic_name == inst.cell.basic_name()
                and components[-1].trans == inst.trans
            ):
                if verbose:
                    print(
                        "Found requested Inst (exact name and origin match): %s"
                        % inst.trans
                    )
                return components[-1]
            ...
            if components[-1].trans == inst.trans:
                if verbose:
                    print(
                        "Found requested Inst (origin match only, could be the right one): %s"
                        % inst.trans
                    )
...
def identify_nets(self, verbose=False):
    ...
    for c1 in components:
        for c2 in components[c1.idx + 1 : len(components)]:
            if verbose:
                print(
                    " - Components: [%s-%s], [%s-%s].  Pins: %s, %s"
                    % (c1.component, c1.idx, c2.component, c2.idx, c1.pins, c2.pins)
                )

            if c1.polygon.bbox().overlaps(
                c2.polygon.bbox()
            ) or c1.polygon.bbox().touches(c2.polygon.bbox()):
                ...
                for p1 in [p for p in c1.pins if p.type == _globals.PIN_TYPES.OPTICAL]:
                    for p2 in [
                        p for p in c2.pins if p.type == _globals.PIN_TYPES.OPTICAL
                    ]:
                        if verbose:
                            print(
                                " - Components, pins: [%s-%s, %s, %s, %s], [%s-%s, %s, %s, %s]; difference: %s"
                                % (
                                    c1.component,
                                    c1.idx,
                                    p1.pin_name,
                        ...
                        if check1 and check2:  # found connected pins:
                            ...
                            if verbose:
                                print(
                                    " - pin-pin, net: %s, component, pin: [%s-%s, %s, %s, %s], [%s-%s, %s, %s, %s]"
                                    % (
                                        net_idx,
                                        c1.component,
                                        c1.idx,
        ...
        if iter1.shape().is_box():
            ...
            if iter1.cell().basic_name() == ("Lumerical_INTERCONNECT_Detector"):
                ...
                if len(components_IO) > 1:
                    raise Exception(
                        "Error - more than 1 optical IO connected to the detector."
                    )
            ...
            if iter1.cell().basic_name() == ("Lumerical_INTERCONNECT_Laser"):
                n_IO += 1
                # *** todo read parameters from Text labels rather than PCell:
                wavelength_start = subcell.pcell_parameters_by_name()[
                    "wavelength_start"
                ]
                ...
                orthogonal_identifier = subcell.pcell_parameters_by_name()[
                    "orthogonal_identifier"
                ]
                ignoreOpticalIOs = subcell.pcell_parameters_by_name()[
                    "ignoreOpticalIOs"
                ]
                if verbose:
                    print(
                ...
                if len(components_IO) > 1:
                    raise Exception(
                        "Error - more than 1 optical IO connected to the laser."
                    )
                if len(components_IO) == 0:
...
def get_LumericalINTERCONNECT_analyzers_from_opt_in(
    self, components, verbose=None, opt_in_selection_text=[]
):
    """
    ...
    if not (components_sorted):
        ...
        warning.setStandardButtons(pya.QMessageBox.Ok)
        warning.setText(
            "To run a simulation, you need to have optical IO in the layout."
        )
        pya.QMessageBox_StandardButton(warning.exec_())
        return False, False, False, False, False, False, False, False

    dist_optin_c = (
        components_sorted[0].trans.disp.to_p().distance(pya.Point(t.x, t.y).to_dtype(1))
    )
    if verbose:
        print(
    ...
    if (
        dist_optin_c
        > float(DFT["design-for-test"]["opt_in"]["max-distance-to-grating-coupler"])
        * 1000
    ):
        warning = pya.QMessageBox()
        warning.setStandardButtons(pya.QMessageBox.Ok)
        warning.setText(
            "To run a simulation, you need to have an opt_in label with %s microns from the nearest grating coupler"
    ...
    detector_GCs = [
        ...
        for c in components
        if [p for p in c.pins if p.type == _globals.PIN_TYPES.OPTICALIO]
        if (c.trans.disp - components_sorted[0].trans.disp).to_p() != pya.DPoint(0, 0)
    ]
    if verbose:
        print(
            "   N=%s, detector GCs: %s"
    ...
    vect_optin_GCs = [
        (c.trans.disp - components_sorted[0].trans.disp).to_p() for c in detector_GCs
    ]
    ...
    for i in range(len(tunable_lasers)):
        if (
            tunable_lasers[i]["wavelength"] == opt_in_dict[0]["wavelength"]
            and tunable_lasers[i]["polarization"] == opt_in_dict[0]["pol"]
        ):
            wavelength_start, wavelength_stop, wavelength_points = (
                float(tunable_lasers[i]["wavelength-start"]),
                float(tunable_lasers[i]["wavelength-stop"]),
                int(tunable_lasers[i]["wavelength-points"]),
            )
    if not ("wavelength_start" in locals()):
        warning = pya.QMessageBox()
        warning.setStandardButtons(pya.QMessageBox.Ok)
        warning.setText(
            "No laser at %s nm is available. Tunable laser definition is in the technology's DFT.xml file."
    ...
    for d in list(
        range(
            int(DFT["design-for-test"]["grating-couplers"]["detectors-above-laser"])
            + 0,
            0,
            -1,
        )
        ...
        range(
            -1,
            -int(DFT["design-for-test"]["grating-couplers"]["detectors-below-laser"])
            - 1,
            -1,
        )
    ):
        if (
            ...
            p = [
                p
                for p in detector_GCs[index].pins
                if p.type == _globals.PIN_TYPES.OPTICALIO
            ]
...
def spice_netlist_export(self, verbose=False, opt_in_selection_text=[]):
    ...
    laser_component = [
        ...
        if any(
            [
                p
                for p in c.pins
                if p.type == _globals.PIN_TYPES.OPTICALIO and "laser" in p.pin_name
            ]
        )
    ]
    ...
    if not components:
        pya.MessageBox.warning(
            "Error: netlist extraction",
            "Error: netlist extraction. No components found connected to opt_in label.",
            pya.MessageBox.Ok,
        )
        return "", "", 0, []
    ...
    print(
        "Scaling for Lumerical INTERCONNECT schematic: %s" % Lumerical_schematic_scaling
    )
    ...
    for c in components:
        for idx, p in enumerate(c.pins):
            if p.type == _globals.PIN_TYPES.ELECTRICAL:
                if idx > 0:
                    if p.pin_name == c.pins[idx - 1].pin_name:
                        continue  # Skip pins that have exactly the same name (assume they are internally connected in the component)
        ...
        for c in components:
            for p in c.pins:
                if p.type == _globals.PIN_TYPES.ELECTRICAL:
                    ...
                    DCsources = (
                        "N1" + NetName + ' "dc source" amplitude=0 sch_x=-2 sch_y=0\n'
                    )
    ...
    for c in components:
        ...
        if component1.find(" ") >= 0 and component1.find('"') == -1:
            text_subckt += ' %s %s "%s" ' % (
                component1.replace(" ", "_") + "_" + str(c.idx),
                nets_str,
                component1,
            )
        else:
            text_subckt += " %s %s %s " % (
                component1.replace('"', "").replace(" ", "_") + "_" + str(c.idx),
                nets_str,
                component1,
            )
        if c.library != None:
        ...
        text_subckt += "%s lay_x=%s lay_y=%s sch_x=%s sch_y=%s %s%s\n" % (
            ...
            eng_str(x * 1e-6),
            eng_str(y * 1e-6),
            eng_str(x * Lumerical_schematic_scaling),
            eng_str(y * Lumerical_schematic_scaling),
            rotate,
            flip,
        )
...
def check_components_models():
    ...
    if not all([c.has_model() for c in components]):
        ...
        components_havemodels = [
            [c.has_model(), c.component, c.instance] for c in components
        ]
        missing_models = []
        ...
        v = pya.MessageBox.warning(
            "All ok",
            "All components have models. Ok to simulate the circuit.",
            pya.MessageBox.Ok,
        )
...
def pinPoint(self, pin_name, verbose=False):
    ...
    if pins:
        ...
        if not matched_pins:
            raise Exception(
                "Did not find matching pin (%s), in the components list of pins (%s)."
                % (pin_name, [p.pin_name for p in pins])
            )
            return
...
pya.Cell.get_LumericalINTERCONNECT_analyzers_from_opt_in = (
    get_LumericalINTERCONNECT_analyzers_from_opt_in
)
...
def find_pins(self, verbose=False):
    ...
    return [
        pin.transform(self.cplx_trans) for pin in self.cell.find_pins(verbose)[0]
    ], errors
...
def pinPoint(self, pin_name, verbose=False):
    ...
    if pins:
        ...
        if p:
            ...
            print(
                "pinPoint, not found: %s, other pins: %s"
                % (pin_name, [p.pin_name for p in pins])
            )
            return Point(0, 0)
    else:
...
def find_pin(self, pin_name, verbose=False):
    ...
    if pins:
        ...
        if p:
            if len(p) > 1:
                raise Exception(
                    'Multiple Pins with name "%s" found in cell "%s"'
                    % (pin_name, self.cell.basic_name())
                )
            return p[0]
        else:
            raise Exception(
                'Pin with name "%s" not found in cell "%s"'
                % (pin_name, self.cell.basic_name())
            )
    else:
...
def move_up_hierachy(self):
    ...
    try:
        ...
        parent_inst = next(each_parent_inst).inst()
        raise Exception(
            "move the cell up one hierarchy works only when the parent cell is instantiated only once."
        )
