...
def pointlist_to_turtle(pointlist):
    ...
    for i in range(1, len(pointlist) - 1):
        ...
        turtle.append(
            int(angle_b_vectors(pts[i] - pts[i - 1], pts[i + 1] - pts[i]) + 90) % 360
            - 90
        )
    return turtle
...
def connect_pins_with_waveguide(
    instanceA,
    pinA,
    instanceB,
    pinB,
    ...
    * works for instances that are sub-circuits with many components, but one unique pin name
    ...
    if instanceA.parent_cell != instanceB.parent_cell:
        ...
        if (
            instanceA.parent_cell.parent_cells()
            > 1 | instanceA.parent_cell.parent_cells()
            > 1
        ):
            raise Exception(
                "connect_pins_with_waveguide function only supports routing where each instance only has only one parent cell."
            )
        ...
        if instanceA.parent_cell.parent_cells() == 1:
            ...
            try:
                ...
                next(each_parent_inst).inst()
                raise Exception(
                    "connect_pins_with_waveguide function only supports routing where each instance is only instantiated once."
                )
        ...
        if len(common_cell) == 0:
            raise Exception(
                "connect_pins_with_waveguide function could not find a common parent for the two instances."
            )
        cell = common_cell[0]
    ...
    if componentA == []:
        ...
        print(
            "all found components A: instance variable: %s"
            % ([n.instance for n in instanceA.parent_cell.find_components()])
        )
        print(
            "all found components A: component variable: %s"
            % ([n.component for n in instanceA.parent_cell.find_components()])
        )
        raise Exception(
            "Component '%s' not found. \nCheck that the component is correctly built (DevRec and PinRec layers). \nTry SiEPIC > Layout > Show Selected Component Information for debugging."
            % instanceA.cell.name
        )
    if componentB == []:
        ...
        print(
            "all found components B: instance variable: %s"
            % ([n.instance for n in instanceB.parent_cell.find_components()])
        )
        print(
            "all found components B: component variable: %s"
            % ([n.component for n in instanceB.parent_cell.find_components()])
        )
        raise Exception(
            "Component '%s' not found. \nCheck that the component is correctly built (DevRec and PinRec layers). \nTry SiEPIC > Layout > Show Selected Component Information for debugging."
            % instanceB.cell.name
        ...
        try:
            if cpinA == []:
                if re.findall(r"\d+", pinA):
                    cpinA = [
                        p
                        for p in componentA.pins
                        if re.findall(r"\d+", pinA)[0] in p.pin_name
                    ]
            if cpinB == []:
                if re.findall(r"\d+", pinB):
                    cpinB = [
                        p
                        for p in componentB.pins
                        if re.findall(r"\d+", pinB)[0] in p.pin_name
                    ]
        except:
    ...
    if cpinA == []:
        try:
            ...
            error_message = (
                "SiEPIC-Tools, in function connect_pins_with_waveguide: Pin (%s) not found in componentA (%s). Available pins: %s"
                % (pinA, componentA.component, [p.pin_name for p in componentA.pins])
            )
            if _globals.Python_Env == "KLayout_GUI":
                question = pya.QMessageBox().setStandardButtons(pya.QMessageBox.Ok)
                question.setText(
                    "SiEPIC-Tools scripted layout, requested pin not found"
                )
    ...
    if cpinB == []:
        try:
            ...
            error_message = (
                "SiEPIC-Tools, in function connect_pins_with_waveguide: Pin (%s) not found in componentB (%s). Available pins: %s"
                % (pinB, componentB.component, [p.pin_name for p in componentB.pins])
            )
            if _globals.Python_Env == "KLayout_GUI":
                question = pya.QMessageBox().setStandardButtons(pya.QMessageBox.Ok)
                question.setText(
    ...
    if not (waveguide):
        ...
        if "compound_waveguide" in waveguide:
            waveguide = [
                w
                for w in waveguides
                if w["name"] == waveguide["compound_waveguide"]["singlemode"]
            ]
    ...
    waveguide_component = [
        c for c in waveguide["component"] if c["layer"] == "Waveguide"
    ]
    ...
    if turtle_A != None:
        vector = pya.CplxTrans(1, directionA, False, 0, 0).trans(
            pya.Vector(to_itype(turtle_A[0], dbu), 0)
        )
        points_fromA.append(points_fromA[-1] + vector)
        for i in range(floor(len(turtle_A) / 2) - 1):
            directionA = (directionA + turtle_A[i * 2 + 1]) % 360
            vector = pya.CplxTrans(1, directionA, False, 0, 0).trans(
                pya.Vector(to_itype(turtle_A[i * 2 + 2], dbu), 0)
            )
            points_fromA.append(points_fromA[-1] + vector)
    if turtle_B != None:
        vector = pya.CplxTrans(1, directionB, False, 0, 0).trans(
            pya.Vector(to_itype(turtle_B[0], dbu), 0)
        )
        points_fromB.append(points_fromB[-1] + vector)
        for i in range(floor(len(turtle_B) / 2) - 1):
            directionB = (directionB + turtle_B[i * 2 + 1]) % 360
            vector = pya.CplxTrans(1, directionB, False, 0, 0).trans(
                pya.Vector(to_itype(turtle_B[i * 2 + 2], dbu), 0)
            )
            points_fromB.append(points_fromB[-1] + vector)
    ...
    if (directionB - directionA - 90) % 360 in [0, 180]:
        ...
        if directionA == 0:
            if (
                directionB == 270
                and points_fromB[-1].y > points_fromA[-1].y
                and points_fromB[-1].x > points_fromA[-1].x
            ) or (
                directionB == 90
                and points_fromB[-1].y < points_fromA[-1].y
                and points_fromB[-1].x > points_fromA[-1].x
            ...
            if points_fromA[-1].y != points_fromB[-1].y:
                ...
                if directionA == 180:
                    x = min(
                        points_fromA[-1].x,
                        points_fromB[-1].x,
                        points_fromA[-2].x - 1 * radius,
                        points_fromB[-2].x - 1 * radius,
                    )
                else:
                    x = max(
                        points_fromA[-1].x,
            ...
            if points_fromA[-1].x != points_fromB[-1].x:
                ...
                if directionA == 270:
                    y = min(
                        points_fromA[-1].y,
                        points_fromB[-1].y,
                        points_fromA[-2].y - 1 * radius,
                        points_fromB[-2].y - 1 * radius,
                    )
                else:
                    y = max(
                        points_fromA[-1].y,
    ...
    if wg_pcell == None:
        raise Exception(
            "problem! cannot create Waveguide PCell from library: %s" % technology_name
        )
    inst = cell.insert(
...
def path_to_waveguide2(
    ...
    if params is None:
        if verbose:
            print(
                "SiEPIC.scripts path_to_waveguide2(): No parameters returned (user pressed Cancel); returning"
            )
        return
    if verbose:
    ...
    if verbose:
        print(
            "SiEPIC.scripts path_to_waveguide2(): selected_paths = %s"
            % (selected_paths)
        )
    ...
    for obj in selected_paths:
        ...
        try:
            pcell = ly.create_cell(
                ...
                {"path": Dpath, "waveguide_type": waveguide_type},
            )
            if "waveguide_type" not in pcell.pcell_parameters_by_name():
                pcell.delete()
                pcell = 0
                print(
                    "SiEPIC.scripts.path_to_waveguide2(): legacy waveguide PCell does not have 'waveguide_type' parameter"
                )
            else:
                if verbose:
                    print(
                        "SiEPIC.scripts.path_to_waveguide2(): Waveguide from %s, %s"
        ...
        if not pcell:
            if "DevRec" not in [wg["layer"] for wg in params["wgs"]]:
                width_devrec = (
                    max([wg["width"] for wg in params["wgs"]])
                    + _globals.WG_DEVREC_SPACE * 2
                )
                params["wgs"].append(
                    {"width": width_devrec, "layer": "DevRec", "offset": 0.0}
                )
        ...
        if not pcell:
            try:
                ...
                print(
                    "SiEPIC.scripts.path_to_waveguide2(): Waveguide from SiEPIC General, %s"
                    % pcell
                )
            except:
                pass
...
def path_to_waveguide(
    ...
    if params is None:
        if verbose:
            print(
                "SiEPIC.scripts path_to_waveguide(): No parameters returned (user pressed Cancel); returning"
            )
    ...
    if verbose:
        print(
            "SiEPIC.scripts path_to_waveguide(): selected_paths = %s" % (selected_paths)
        )
    #        print("SiEPIC.scripts path_to_waveguide(): selected_paths = %s; time = %s" % (selected_paths, time.perf_counter()-time0))
    selection = []
    ...
    for obj in selected_paths:
        ...
        if "CML" in params.keys() and "model" in params.keys():
            try:
                ...
                print(
                    "SiEPIC.scripts.path_to_waveguide(): Waveguide from %s, %s"
                    % (TECHNOLOGY["technology_name"], pcell)
                )
            #            print("SiEPIC.scripts.path_to_waveguide(): Waveguide from %s, %s; time = %s" %
            #                  (TECHNOLOGY['technology_name'], pcell, time.perf_counter()-time0))
        ...
        if not pcell:
            try:
                ...
                print(
                    "SiEPIC.scripts.path_to_waveguide(): Waveguide from SiEPIC General, %s"
                    % pcell
                )
            except:
                pass
...
def roundpath_to_waveguide(verbose=False):
    ...
    for o in object_selection:
        ...
        if o.is_cell_inst():
            ...
            if c.basic_name() in Waveguide_Types and c.is_pcell_variant():
                ...
                if verbose:
                    print(
                        "Selected object is a GUIDING_LAYER in %s on Layer %s."
        ...
        if is_ROUNDPATH == True:
            ...
            if KLAYOUT_VERSION > 24:
                ...
                v = pya.MessageBox.warning(
                    "KLayout 0.25 or higher required.",
                    "ROUND_PATH to Waveguide is implemented using KLayout 0.25 or higher functionality.",
                    pya.MessageBox.Ok,
                )
                return
            ...
            to_delete.append(
                oinst
            )  # delete the instance; leaves behind the cell if it's not used
    ...
    if not (is_ROUNDPATH):
        v = pya.MessageBox.warning(
            "No ROUND_PATH selected",
            "No ROUND_PATH selected.\nPlease select a ROUND_PATH. \nIt will get converted to a path.",
            pya.MessageBox.Ok,
        )
...
def waveguide_to_path(cell=None, save_waveguide_type=True):
    ...
    for obj in waveguides:
        ...
        if KLAYOUT_VERSION > 24:
            ...
            selection[-1].shape = cell.shapes(ly.layer(TECHNOLOGY["Waveguide"])).insert(
                path.transformed(
                    pya.Trans(waveguide.trans.disp.x, waveguide.trans.disp.y)
                )
            )
        ...
        if save_waveguide_type:
            if "waveguide_type" in waveguide.cell.pcell_parameters_by_name():
                selection[-1].shape.set_property(
                    1, waveguide.cell.pcell_parameters_by_name()["waveguide_type"]
                )
...
def waveguide_length():
    ...
    if len(selection) == 1:
        ...
        length = (
            float(cell.find_components()[0].params.split(" ")[0].split("=")[1]) * 1e6
        )
        pya.MessageBox.warning(
            "Waveguide Length", "Waveguide length (um): %s" % length, pya.MessageBox.Ok
        )
    else:
        pya.MessageBox.warning(
            "Selection is not a waveguide",
            "Select one waveguide you wish to measure.",
            pya.MessageBox.Ok,
        )
...
def waveguide_length_diff():
    ...
    if len(selection) == 2:
        ...
        length1 = (
            float(cell1.find_components()[0].params.split(" ")[0].split("=")[1]) * 1e6
        )
        length2 = (
            float(cell2.find_components()[0].params.split(" ")[0].split("=")[1]) * 1e6
        )
            ...
            def split_line(line_pts, length):
                ...
                for i in range(len(line_pts)):
                    if i > 0:
                        ...
                        while (
                            distance(curr_pt[0], curr_pt[1], pt_b[0], pt_b[1]) > length
                        ):
                ...
                for idx2 in range(len(comp)):
                    ...
                    if corr_matrix[idx1, idx2] == 0 and corr_matrix[idx2, idx1] == 0:
                        ...
                        if idx1 == idx2:  # if it is the same element
                            ...
                            if (
                                first.basic_name == "Waveguide"
                                or second.basic_name == "Waveguide"
                            ):
                                wgs = [first, second]
                                ...
                                for each in wgs:
                                    if each.basic_name == "Waveguide":
                                        each_path = (
                                            each.cell.pcell_parameters_by_name()["path"]
                                        )
                                        ...
                                        wg_pts.append(
                                            [[each.x, each.y] for each in pts_itr]
                                        )
                        ...
                        s2i = (
                            comp[idx1].basic_name
                            + "_"
                            + str(idx1)
                            + " & "
                            + comp[idx2].basic_name
                            + "_"
                            + str(idx2)
                        )
        ...
        for root, dirnames, filenames in os.walk(
            pya.Application.instance().application_data_path(), followlinks=True
        ):
            [
                paths.append(os.path.join(root, filename))
                for filename in fnmatch.filter(filenames, "MONTECARLO.xml")
        ...
        if mc_exists:
            ...
            for i in range(2):
                for j in range(2):
                    wcovariance_matrix[i][j] = (
                        wsigma * correlation_matrix_w[i][j] * wsigma
                    )
                    tcovariance_matrix[i][j] = (
                        tsigma * correlation_matrix_h[i][j] * tsigma
                    )
            ...
            match = [
                each
                for each in pathsdata
                if (len(each) == 3) and (filename in each[-1])
            ]
            ...
            if len(match) > 0:
                ...
                geovsneff_data = np.load(
                    os.path.join(filedir, filename), allow_pickle=True
                ).flatten()[0]
                ...
                for each_sample in range(nsamples):
                    ...
                    idxx, idxy = np.where(
                        thickness_data == find_nearest(thickness_data, temp_thick1)
                    ), np.where(width_data == find_nearest(width_data, temp_width1))
                    ...
                    idxx, idxy = np.where(
                        thickness_data == find_nearest(thickness_data, temp_thick)
                    ), np.where(width_data == find_nearest(width_data, temp_width))
                    neff2 = neff_data[idxy, idxx].flatten()[0]
                    beta2 = (2 * np.pi * neff2) / lambda_not

                    delta_beta[each_sample] = np.abs(beta1 - beta2)
                    phase_arr[each_sample] = (
                        (beta1 * length1) - (beta2 * length2)
                    ) / np.pi

        pya.MessageBox.warning(
            "Waveguide Length Difference",
            "Difference in waveguide lengths (um): %s" % str(abs(length1 - length2))
            + "\r\n RMS phase error: "
            + str(round(np.std(phase_arr), 3))
            + " pi radians",
            pya.MessageBox.Ok,
        )
        ...
        pya.MessageBox.warning(
            "Selection are not a waveguides",
            "Select two waveguides you wish to measure.",
            pya.MessageBox.Ok,
        )
    ...
    if lv.has_transient_object_selection() == False:
        ...
        if sys.platform.startswith("darwin"):
            v = pya.MessageBox.warning(
                ...
                "Hover the mouse (transient selection) over the object to which you wish to snap to.\nIf it still doesn't work, please ensure that 'Transient mode' selection is enabled in the KLayout menu KLayout - Preferences - Applications - Selection.",
                pya.MessageBox.Ok,
            )
        else:
            v = pya.MessageBox.warning(
                "No transient selection",
                "Hover the mouse (transient selection) over the object to which you wish to snap to.\nIf it still doesn't work, please ensure that 'Transient mode' selection is enabled in the KLayout menu File - Settings - Applications - Selection.",
                pya.MessageBox.Ok,
            )
    else:
        ...
        if len(o_selection) < 1:
            v = pya.MessageBox.warning(
                "No selection",
                "Select the object you wish to be moved.",
                pya.MessageBox.Ok,
            )
        if len(o_selection) > 1:
            v = pya.MessageBox.warning(
                "Too many selected",
                "Select only one object you wish to be moved.",
                pya.MessageBox.Ok,
            )
        else:
            ...
            if o_selection.is_cell_inst() == False:
                v = pya.MessageBox.warning(
                    "No selection",
                    "The selected object must be an instance (not primitive polygons)",
                    pya.MessageBox.Ok,
                )
                ...
                v = pya.MessageBox.warning(
                    "Array",
                    "Selection was an array. \nThe array was 'exploded' (Edit | Selection | Resolve Array). \nPlease select the objects and try again.",
                    pya.MessageBox.Ok,
                )
                # Record a transaction, to enable "undo"
                ...
                v = pya.MessageBox.warning(
                    "Same selection",
                    "We need two different objects: one selected, and one transient (hover mouse over).",
                    pya.MessageBox.Ok,
                )
            else:
                ...
                print(
                    "all pins_transient (x,y): %s"
                    % [
                        [point.x, point.y]
                        for point in [pin.center for pin in pins_transient]
                    ]
                )
                print(
                    "all pins_selection (x,y): %s"
                    % [
                        [point.x, point.y]
                        for point in [pin.center for pin in pins_selection]
                    ]
                )
                ...
                pin_pairs = sorted(
                    [
                        ...
                        if (abs(pin_t.rotation - pin_s.rotation) % 360 == 180)
                        and pin_t.type == _globals.PIN_TYPES.OPTICAL
                        and pin_s.type == _globals.PIN_TYPES.OPTICAL
                    ],
                ...
                if pin_pairs:
                    print(
                        "shortest pins_transient & pins_selection (x,y): %s"
                        % [
                            [point.x, point.y]
                            for point in [pin.center for pin in pin_pairs[0]]
                        ]
                    )
                    print(
                        "shortest distance: %s"
                        % pin_pairs[0][0].center.distance(pin_pairs[0][1].center)
                    )

                    trans = pya.Trans(
                        pya.Trans.R0, pin_pairs[0][0].center - pin_pairs[0][1].center
                    )
                    ...
                    v = pya.MessageBox.warning(
                        "Snapping failed",
                        "Snapping failed. \nNo matching pins found. \nNote that pins must have exactly matching orientations (180 degrees)",
                        pya.MessageBox.Ok,
                    )
                    return
...
def connect_cell(
    ...
    mirror=False,
    verbose=False,
    translation=pya.Trans.R0,
    relaxed_pinnames=False,
):
    """
    ...
    # Find the two components:
    componentA = instanceA.parent_cell.find_components(
        cell_selected=instanceA.cell, inst=instanceA
    )
    ...
    if componentA == []:
        ...
        if componentA == []:
            if _globals.Python_Env == "KLayout_GUI":
                question = pya.QMessageBox().setStandardButtons(pya.QMessageBox.Ok)
                question.setText(
                    "SiEPIC-Tools scripted layout, requested component not found"
                )
                question.setInformativeText(
                    "Component instanceA not found: %s, %s"
                    % (instanceA, instanceA.cell.name)
                )
                pya.QMessageBox_StandardButton(question.exec_())
                return
            else:
    ...
    if componentB == []:
        if _globals.Python_Env == "KLayout_GUI":
            ...
            question.setInformativeText(
                "Component cellB not found: %s, %s" % (cellB, cellB.name)
            )
            pya.QMessageBox_StandardButton(question.exec_())
            return
        else:
    ...
    def error_pinA(pinA, componentA):
        ...
        error_message = (
            "SiEPIC-Tools, in function connect_cell: PinA (%s) not found in componentA (%s). Available pins: %s.\n%s"
            % (
                pinA,
                componentA.component,
                [p.pin_name for p in componentA.pins],
                getframeinfo(stack()[2][0]),
            )
        )
        if _globals.Python_Env == "KLayout_GUI":
            question = pya.QMessageBox().setStandardButtons(pya.QMessageBox.Ok)
    ...
    def error_pinB(pinB, componentB):
        ...
        error_message = (
            "SiEPIC-Tools, in function connect_cell: PinB (%s) not found in componentB (%s). Available pins: %s.\n%s"
            % (
                pinB,
                componentB.component,
                [p.pin_name for p in componentB.pins],
                getframeinfo(stack()[2][0]),
            )
        )
        if _globals.Python_Env == "KLayout_GUI":
    ...
    if mirror:
        trans = pya.Trans(
            pya.Trans.R0.M45,
            cpinA.center - cpinB.center * pya.Trans(rotation / 90, False, 0, 0),
  ...
  """
    ...
    instanceB = instanceA.parent_cell.insert(
        pya.CellInstArray(cellB.cell_index(), trans * translation)
    )
...
def delete_extra_topcells(ly, keep_topcell):
    ...
    if type(keep_topcell) != pya.Cell:
        raise Exception(
            "SiEPIC.scripts.delete_extra_topcells: keep_topcell argument was not a cell"
        )
    if keep_topcell in ly.top_cells():
        ly.delete_cells(
            [
                tcell
                for tcell in ly.each_top_cell()
                if tcell != keep_topcell.cell_index()
            ]
        )
...
def calibreDRC(params=None, cell=None):
    ...
    if not params:
        ...
        params["remote_calibre_rule_deck_main_file"] = CALIBRE["Calibre"][
            "remote_calibre_rule_deck_main_file"
        ]
        params["remote_additional_commands"] = CALIBRE["Calibre"][
            "remote_additional_commands"
        ]
    ...
    with codecs.open(
        os.path.join(local_path, "run_calibre"), "w", encoding="utf-8"
    ) as file:
        cal_script = "#!/bin/tcsh \n"
        cal_script += "source %s \n" % params["calibre"]
    ...
    with codecs.open(
        os.path.join(local_path, "drc.cal"), "w", encoding="utf-8"
    ) as file:
    ...
    if version.find("2.") > -1:
        ...
        out += cmd(
            'cd "%s" && scp "%s" %s:%s' % (local_path, local_file, server, remote_path)
        )[1]
        out += cmd(
            'cd "%s" && scp "%s" %s:%s'
            % (local_path, "run_calibre", server, remote_path)
        )[1]
        out += cmd(
            'cd "%s" && scp "%s" %s:%s' % (local_path, "drc.cal", server, remote_path)
        )[1]
        ...
        out += cmd(
            'cd "%s" && scp %s:%s "%s"'
            % (local_path, server, remote_path + "/drc.rve", results_file)
        )[1]
        ...
        ("Darwin" in platform.system()) | ("Linux" in platform.system())
    ):
        import subprocess
        ...
        try:
            ...
            c = [
                ...
                "%s:%s" % (server, remote_path),
            ]
            print(c)
            out += cmd(c).decode("utf-8")
            c = [
                "scp",
                os.path.join(local_path, "run_calibre"),
                "%s:%s" % (server, remote_path),
            ]
            print(c)
            out += cmd(c).decode("utf-8")
            c = [
                "scp",
                os.path.join(local_path, "drc.cal"),
                "%s:%s" % (server, remote_path),
            ]
            print(c)
            out += cmd(c).decode("utf-8")
            ...
            c = [
                "scp",
                "%s:%s/drc.rve" % (server, remote_path),
                os.path.join(local_path, results_file),
            ]
            print(c)
            out += cmd(c).decode("utf-8")
        ...
        try:
            out += cmd(
                'ssh %s "mkdir -p %s"' % (server, remote_path), shell=True
            ).decode("utf-8")
            out += cmd(
                'cd "%s" && scp "%s" %s:%s'
                % (local_path, local_file, server, remote_path),
                shell=True,
            ).decode("utf-8")
            out += cmd(
                'cd "%s" && scp "%s" %s:%s'
            ...
            out += cmd(
                'cd "%s" && scp "%s" %s:%s'
                % (local_path, "drc.cal", server, remote_path),
                shell=True,
            ).decode("utf-8")
            ...
            out += cmd(
                'ssh %s "cd %s && source run_calibre"' % (server, remote_path),
                shell=True,
            ).decode("utf-8")
    ...
    if os.path.exists(results_pathfile):
        pya.MessageBox.warning(
            "Success",
            "Calibre DRC run complete. Results downloaded and available in the Marker Browser window.",
            pya.MessageBox.Ok,
        )
        ...
        pya.MessageBox.warning(
            "Errors",
            "Something failed during the server Calibre DRC check: %s" % out,
            pya.MessageBox.Ok,
        )
...
def auto_coord_extract():
    ...
    cell = (
        pya.Application.instance().main_window().current_view().active_cellview().cell
    )
    text_out, opt_in = find_automated_measurement_labels(cell)
...
def find_SEM_labels_gui(topcell=None, LayerSEMN=None, MarkersInTopCell=False):
    ...
    if not LayerSEMN:
        ...
        if "SEM" in TECHNOLOGY:
            ...
            if layers != []:
                ...
                v = pya.MessageBox.warning(
                    "SEM images",
                    "No 'SEM' layer found in the Technology.",
                    pya.MessageBox.Ok,
                )
                return
    if not topcell:
    ...
    rdb_i = lv.create_rdb(
        "SiEPIC-Tools SEM images: %s technology" % TECHNOLOGY["technology_name"]
    )
    rdb = lv.rdb(rdb_i)
    ...
    while not (iter.at_end()):
        if iter.shape().is_box():
            ...
            if MarkersInTopCell:
                box2 = (
                    iter.shape().box.transformed(iter.itrans()).to_dtype(dbu)
                )  # when added to the top cell
            else:
    ...
    if rdb.num_items() > 0:
        v = pya.MessageBox.warning(
            "SEM images",
            "%s SEM images found.  \nPlease review SEM images using the 'Marker Database Browser'."
            % rdb.num_items(),
            pya.MessageBox.Ok,
        )
        ...
        v = pya.MessageBox.warning(
            "SEM images", "No SEM images found.", pya.MessageBox.Ok
        )
    pya.Application.instance().main_window().redraw()
...
def calculate_area(ly=None, cell=None):
    ...
    try:
        total = (
            cell.each_shape(ly.layer(TECHNOLOGY["FloorPlan"])).__next__().polygon.area()
        )
    except:
        total = 1e99
    ...
    for l in ly.layer_indexes():
        ...
        if total == 1e99:
            ...
            text += (
                "Layer: %s, area: %.5g mm^2 (%.5g micron^2), Percentage of floorplan: %.3g %%\n"
                % (
                    ly.get_info(l).to_s(),
                    area / 1e6 * dbu * dbu,
                    area / 1e6,
    ...
    if Python_Env == "KLayout_GUI":
        p.destroy
        v = pya.MessageBox.warning("Waveguide area.", text, pya.MessageBox.Ok)
...
def trim_netlist(nets, components, selected_component, verbose=None):
    ...
    trimmed_net = []
    net_idx = [
        [each.pins[0].component.idx, each.pins[1].component.idx] for each in nets
    ]
    ...
    while count < (len_net_idx - 1):
        for i in range(
            count + 1, len_net_idx
        ):  # i keep track of nets from next net to last net
            ...
            if (
                len(first_set.intersection(second_set)) > 0
            ):  # if there are common elements between two sets
    ...
    for net in net_idx:
        if selected.idx in net:
            ...
            trimmed_nets = [
                ...
                for each in nets
                if (
                    each.pins[0].component.idx in net
                    or each.pins[1].component.idx in net
                )
            ]
            if verbose:
...
def layout_check(cell=None, verbose=False):
    ...
    if not components:
        v = pya.MessageBox.warning(
            "Errors",
            "No components found (using SiEPIC-Tools DevRec and PinRec definitions).",
            pya.MessageBox.Ok,
        )
        return
    ...
    # Create a Results Database
    rdb_i = lv.create_rdb(
        "SiEPIC-Tools Verification: %s technology" % TECHNOLOGY["technology_name"]
    )
    rdb = lv.rdb(rdb_i)
    rdb.top_cell_name = cell.name
    rdb_cell = rdb.create_cell(cell.name)
    ...
    rdb_cat_id_wg_radius.description = (
        "Not enough space to accommodate the desired bend radius for the waveguide."
    )
    rdb_cat_id_wg_bendpts = rdb.create_category(rdb_cat_id_wg, "Bend points")
    rdb_cat_id_wg_bendpts.description = (
        "Waveguide bend should have more points per circle."
    )
    ...
    rdb_cat_id_comp_overlap = rdb.create_category(
        rdb_cat_id_comp, "Overlapping component"
    )
    rdb_cat_id_comp_overlap.description = (
        "Overlapping components (defined as overlapping DevRec layers; touching is ok)"
    )
    rdb_cat_id_comp_shapesoutside = rdb.create_category(
        rdb_cat_id_comp, "Shapes outside component"
    )
    ...
    if 0:
        ...
        rdb_cat_id_sim_nomodel = rdb.create_category(
            rdb_cat_id, "Missing compact model"
        )
    ...
    if DFT:
        ...
        rdb_cat_id_optin_unique.description = (
            "Automated test opt_in labels should be unique."
        )
        rdb_cat_id_optin_missing = rdb.create_category(
            rdb_cat_id, "opt_in label: missing"
        )
        ...
        rdb_cat_id_optin_toofar = rdb.create_category(
            rdb_cat_id, "opt_in label: too far away"
        )
        ...
        rdb_cat_id_optin_wavelength = rdb.create_category(
            rdb_cat_id, "opt_in label: wavelength"
        )
        if type(DFT["design-for-test"]["tunable-laser"]) == list:
            DFT_wavelengths = [
                w["wavelength"] for w in DFT["design-for-test"]["tunable-laser"]
            ]
        else:
            DFT_wavelengths = DFT["design-for-test"]["tunable-laser"]["wavelength"]
        rdb_cat_id_optin_wavelength.description = (
            "Automated test opt_in labels must have a wavelength for a laser specified in the DFT.xml file: %s.  \n\nDetails on the format for the opt_in labels can be found at https://github.com/lukasc-ubc/SiEPIC-Tools/wiki/SiEPIC-Tools-Menu-descriptions#connectivity-layout-check"
            % DFT_wavelengths
        )
        if type(DFT["design-for-test"]["tunable-laser"]) == list:
            DFT_polarizations = [
                p["polarization"] for p in DFT["design-for-test"]["tunable-laser"]
            ]
        else:
            if "polarization" in DFT["design-for-test"]["tunable-laser"]:
                DFT_polarizations = DFT["design-for-test"]["tunable-laser"][
                    "polarization"
                ]
            else:
        ...
        rdb_cat_id_optin_polarization.description = (
            "Automated test opt_in labels must have a polarization as specified in the DFT.xml file: %s. \n\nDetails on the format for the opt_in labels can be found at https://github.com/lukasc-ubc/SiEPIC-Tools/wiki/SiEPIC-Tools-Menu-descriptions#connectivity-layout-check"
            % DFT_polarizations
        )
        ...
        rdb_cat_id_GCorient = rdb.create_category(
            rdb_cat_id, "Grating coupler orientation"
        )
        rdb_cat_id_GCorient.description = "The grating coupler is not oriented (rotated) the correct way for automated testing."
        rdb_cat_id_GCarrayconfig = rdb.create_category(
            rdb_cat_id, "Fibre array configuration"
        )
        array_angle = (
            float(DFT["design-for-test"]["grating-couplers"]["gc-array-orientation"])
        ) % 360.0
        if array_angle == 0:
            dir1 = " right of "
            dir2 = " left of "
        ...
        rdb_cat_id_GCarrayconfig.description = (
            "Circuit must be connected such that there is at most %s Grating Coupler(s) %s the opt_in label (laser injection port) and at most %s Grating Coupler(s) %s the opt_in label. \nGrating couplers must be on a %s micron pitch, %s arranged."
            % (
                int(
                    DFT["design-for-test"]["grating-couplers"]["detectors-above-laser"]
                ),
                dir1,
                int(
                    DFT["design-for-test"]["grating-couplers"]["detectors-below-laser"]
                ),
                dir2,
                float(DFT["design-for-test"]["grating-couplers"]["gc-pitch"]),
                dir3,
            )
        )
    ...
    for i in range(0, len(components)):
        ...
        if c.basic_name == "Waveguide" and c.cell.is_pcell_variant():
            ...
            if not Dpath.is_manhattan_endsegments():
                rdb_item = rdb.create_item(
                    rdb_cell.rdb_id(), rdb_cat_id_wg_manhattan.rdb_id()
                )
                rdb_item.add_value(pya.RdbItemValue(Dpath))
        ...
        for pin in c.pins:
            if pin.type == _globals.PIN_TYPES.OPTICAL and pin.net.idx == None:
                ...
                if verbose:
                    print(
                        " - Found disconnected pin, type %s, at (%s)"
                        % (pin.type, pin.center)
                    )
                rdb_item = rdb.create_item(
                    rdb_cell.rdb_id(), rdb_cat_id_discpin.rdb_id()
                )
        ...
        for i2 in range(i + 1, len(components)):
            ...
            if polygon_and:
                print(
                    " - Found overlapping components: %s, %s"
                    % (c.component, c2.component)
                )
                rdb_item = rdb.create_item(
                    rdb_cell.rdb_id(), rdb_cat_id_comp_overlap.rdb_id()
                )
        ...
        if DFT:
            ...
            if c.basic_name:
                ...
                for gc in DFT["design-for-test"]["grating-couplers"][
                    "gc-orientation"
                ].keys():
                    DFT_GC_angle = int(
                        DFT["design-for-test"]["grating-couplers"]["gc-orientation"][gc]
                    )
                    if ci.startswith(gc) and c.trans.angle != DFT_GC_angle:
                        if verbose:
                            print(
                                " - Found DFT error, GC facing the wrong way: %s, %s"
                                % (c.component, c.trans.angle)
                            )
                        rdb_item = rdb.create_item(
                            rdb_cell.rdb_id(), rdb_cat_id_GCorient.rdb_id()
                        )
                        rdb_item.add_value(
                            pya.RdbItemValue(
                                "Cell %s should be %s degrees" % (ci, DFT_GC_angle)
                            )
                        )
                        rdb_item.add_value(pya.RdbItemValue(c.polygon.to_dtype(dbu)))
        ...
        if not c.has_model() and 0:
            ...
            rdb_item = rdb.create_item(
                rdb_cell.rdb_id(), rdb_cat_id_sim_nomodel.rdb_id()
            )
            rdb_item.add_value(pya.RdbItemValue(c.polygon.to_dtype(dbu)))
    ...
    """
        ...
        for ti1 in range(0, len(opt_in)):
            if "opt_in" in opt_in[ti1]:
                ...
                for ti2 in range(ti1 + 1, len(opt_in)):
                    if "opt_in" in opt_in[ti2]:
                        if opt_in[ti1]["opt_in"] == opt_in[ti2]["opt_in"]:
                            if verbose:
                                print(
                                    " - Found DFT error, non unique text labels: %s, %s, %s"
                                    % (t.string, t.x, t.y)
                                )
                            rdb_item = rdb.create_item(
                                rdb_cell.rdb_id(), rdb_cat_id_optin_unique.rdb_id()
                            )
                            rdb_item.add_value(pya.RdbItemValue(t.string))
                            rdb_item.add_value(
                                pya.RdbItemValue(pya.Polygon(box).to_dtype(dbu))
                            )
                ...
                if not opt_in[ti1]["wavelength"] in DFT_wavelengths:
                    ...
                    rdb_item = rdb.create_item(
                        rdb_cell.rdb_id(), rdb_cat_id_optin_wavelength.rdb_id()
                    )
                ...
                components_sorted = sorted(
                    [
                        c
                        for c in components
                        if [p for p in c.pins if p.type == _globals.PIN_TYPES.OPTICALIO]
                    ],
                ...
                if components_sorted:
                    ...
                    if verbose:
                        print(
                            " - Found opt_in: %s, nearest GC: %s.  Locations: %s, %s. distance: %s"
                            % (
                                opt_in[ti1]["Text"],
                                components_sorted[0].instance,
                                components_sorted[0].center,
                                pya.Point(t.x, t.y),
                                dist_optin_c * dbu,
                            )
                        )
                    if (
                        dist_optin_c
                        > float(
                            DFT["design-for-test"]["opt_in"][
                                "max-distance-to-grating-coupler"
                            ]
                        )
                        * 1000
                    ):
                        if verbose:
                            print(
                                " - opt_in label too far from the nearest grating coupler: %s, %s"
                                % (components_sorted[0].instance, opt_in[ti1]["opt_in"])
                            )
                        rdb_item = rdb.create_item(
                            rdb_cell.rdb_id(), rdb_cat_id_optin_toofar.rdb_id()
                        )
                        rdb_item.add_value(
                            pya.RdbItemValue(pya.Polygon(box).to_dtype(dbu))
                        )
                    ...
                    components_connected_opt_in = (
                        components_connected_opt_in + trimmed_components
                    )
                    detector_GCs = [
                        c
                        for c in trimmed_components
                        if [p for p in c.pins if p.type == _globals.PIN_TYPES.OPTICALIO]
                        if (c.trans.disp - components_sorted[0].trans.disp).to_p()
                        != pya.DPoint(0, 0)
                    ]
                    if verbose:
                        print(
                    ...
                    vect_optin_GCs = [
                        (c.trans.disp - components_sorted[0].trans.disp).to_p()
                    ...
                    array_angle = float(
                        DFT["design-for-test"]["grating-couplers"][
                            "gc-array-orientation"
                        ]
                    )
                    pitch = float(
                        DFT["design-for-test"]["grating-couplers"]["gc-pitch"]
                    )
                    ...
                    for d in list(
                        ...
                        if (
                            pya.DPoint(d * sx * pitch * 1000, d * sy * pitch * 1000)
                            in vect_optin_GCs
                        ):
                            del_index = vect_optin_GCs.index(
                                pya.DPoint(d * sx * pitch * 1000, d * sy * pitch * 1000)
                            )
                    ...
                    for vi in range(0, len(vect_optin_GCs)):
                        ...
                        rdb_item.add_value(
                            pya.RdbItemValue(detector_GCs[vi].polygon.to_dtype(dbu))
                        )
                        rdb_item.add_value(
                            pya.RdbItemValue(pya.Polygon(box).to_dtype(dbu))
                        )
        ...
        while components_without_opt_in and i < 50:
            ...
            components_GCs = [
                c
                for c in components_without_opt_in
                if [p for p in c.pins if p.type == _globals.PIN_TYPES.OPTICALIO]
            ]
            if components_GCs:
                trimmed_nets, trimmed_components = trim_netlist(
                ...
                components_without_opt_in = [
                    c
                    for c in components_without_opt_in
                    if not (c in trimmed_components)
                ]
                i += 1
            else:
                break
    ...
    for n in nets:
        ...
        if n.type == _globals.PIN_TYPES.OPTICAL and not n.idx == None:
            ...
            if pin_paths[0].width != pin_paths[-1].width:
                ...
                r = pya.Region(
                    [
                        pin_paths[0].to_itype(1).polygon(),
                        pin_paths[-1].to_itype(1).polygon(),
                    ]
                )
                polygon_merged = advance_iterator(r.each_merged())
                rdb_item = rdb.create_item(
                    rdb_cell.rdb_id(), rdb_cat_id_mismatchedpin.rdb_id()
                )
                rdb_item.add_value(
                    pya.RdbItemValue(
                        "Pin widths: %s, %s" % (pin_paths[0].width, pin_paths[-1].width)
                    )
                )
    ...
    if rdb.num_items() > 0:
        v = pya.MessageBox.warning(
            "Errors",
            "%s layout errors detected.  \nPlease review errors using the 'Marker Database Browser'."
            % rdb.num_items(),
            pya.MessageBox.Ok,
        )
        lv.show_rdb(rdb_i, cv.cell_index)
        ...
        v = pya.MessageBox.warning(
            "Errors", "No layout errors detected.", pya.MessageBox.Ok
        )
    ...
    text = pya.DText(
        "SiEPIC-Tools verification: %s errors\n%s\nSiEPIC-Tools v%s\ntechnology: %s\n%s\nPython: %s, %s\n%s"
        % (
            rdb.num_items(),
            strftime("%Y-%m-%d %H:%M:%S"),
            SiEPIC.__init__.__version__,
            TECHNOLOGY["technology_name"],
            sys.platform,
            sys.version.split("\n")[0],
            sys.path[0],
...
def open_PDF_files(files, files_list):
    ...
    if sys.platform.startswith("darwin"):
        ...
        if int(sys.version[0]) > 2:
            ...
            subprocess.Popen(
                ["/usr/bin/open", "-n", "-a", "/Applications/Preview.app", files]
            )
        else:
            import commands
...
def fetch_measurement_data_from_github(verbose=None, opt_in_selection_text=[]):
    ...
    if not opt_in_selection_text:
        pya.MessageBox.warning(
            "opt_in labels not found",
            "Error: opt_in labels not found",
            pya.MessageBox.Ok,
        )
        return
    ...
    for ot in opt_in_selection_text:
        ...
        if len(filenames) == 0:
            ...
            if len(filenames) == 0:
                pya.MessageBox.warning(
                    ...
                    "Measurement data not found; searched for: %s on GitHub %s/%s"
                    % (search_for, user, repo),
                    pya.MessageBox.Ok,
                )
                print(" measurement not found!")
                return
        ...
        if len(filenames) == 1:
            ...
            if all_measurements == 0:
                ...
                measurements_text = pya.InputDialog.ask_item(
                    "opt_in selection",
                    "Choose one of the data files for opt_in = %s, to fetch experimental data.\n"
                    % search_for,
                    measurements,
                    0,
                )
        ...
        if not folder_flatten_option:
            ...
            folder_flatten_option = pya.InputDialog.ask_item(
                "folder tree",
                "Do you wish to place all files in the same folder (flatten folder tree), or recreate the folder tree structure?",
                options,
                0,
            )
        ...
        if all_measurements == 1:
            savefilepath = github_get_files(
                user=user,
                repo=repo,
                filename_search=search_for,
            ...
            savefilepath = github_get_files(
                user=user,
                repo=repo,
                filename_search=file_selection[0].replace("." + extension, "."),
                save_folder=tmp_folder,
                include_path=include_path,
                verbose=verbose,
            )
...
def measurement_vs_simulation(verbose=None):
    ...
    for ot in opt_in_selection_text:
        ...
        # simulate:
        circuit_simulation(
            verbose=verbose, opt_in_selection_text=[ot], matlab_data_files=savefilepath
        )
...
def resize_waveguide():
    ...
    if len(selection) > 1 or len(selection) == 0:
        ...
        if path_obj.points <= 3:
            ...
            path_length = (
                float(c.find_components()[0].params.split(" ")[0].split("=")[1]) * 1e6
            )
            ...
            def button(self):
                ...
                dpath = pya.DPath(
                    dpoints, 0.5 * c.layout().dbu
                )  # 0.5 is irrelevant to actual waveguide width
...
def replace_cell(
    ...
    if debug:
        print(
            " - cell replacement for: %s, with cell %s (%s), "
    ...
    if Exact:
        ...
        cells_x = [
            cell for cell in layout.each_cell() if cell.name.find(cell_x_name) == 0
        ]
    ...
    if cell_y_file:
        ...
        if debug:
            print(
                " - checking for cell %s in current layout: %s" % (cell_y_name, cell_y)
            )
        if not cell_y:
            # Load cell_y_name:
            if debug:
        ...
        if not cell_y:
            raise Exception(
                "No cell '%s' found in layout %s." % (cell_y_name, cell_y_file)
            )
        if debug:
            print(
                "   - replacing with cell: %s, from: %s."
                % (cell_y.name, os.path.basename(cell_y_file))
            )
    if cell_y_library:
        cell_y = layout.create_cell(cell_y_name, cell_y_library)
        if not cell_y:
            raise Exception(
                "Cannot import cell %s from library %s" % (cell_y_name, cell_y_library)
            )
        ...
        for c in caller_cells:
            ...
            while inst:
                ...
                if inst.cell.name == cell_x.name:
                    if cell_y.destroyed():
                        print(
                            "   - Warning: cell_y (%s) destroyed, skipping replacement"
                            % (cell_y_name)
                        )
                        print(
                            "   - destroyed status: cell_y - %s, cell_x - %s, cc - %s"
                            % (cell_y.destroyed(), cell_x.destroyed(), cc.destroyed())
                        )
                        print(
                            "   - looking for cell. %s, %s, %s"
                            % (cell_y_name, cell_y, layout.cell(cell_y_name))
                        )
                    ...
                    if inst.is_regular_array():
                        if debug:
                            print(
                                "    - replacing %s in %s, with cell array: %s"
                                % (cell_x.name, cc.name, cell_y.name)
                            )
                        ci = inst.cell_inst
                        cc.replace(
                            inst,
                        ...
                        if debug:
                            print(
                                "    - replacing %s in %s, with cell: %s"
                                % (cell_x.name, cc.name, cell_y.name)
                            )
                        cc.replace(
                            inst, pya.CellInstArray(cell_y.cell_index(), inst.trans)
                        )
                inst = next(itr, None)
...
def svg_from_cell(verbose=True):
    ...
    component = cell.find_components(cell_selected=[selected_instances[0].inst().cell])[
        0
    ]
    ...
    message.setText(
        "Exported SVG file for selected component. File in location: %s." % svg_filename
    )
...
def instantiate_all_library_cells(topcell, progress_bar=True):
    ...
    for lib in pya.Library().library_ids():
        ...
        for n in li.layout().pcell_names():
            ...
            if pcell:
                t = pya.Trans(
                    pya.Trans.R0, x - pcell.bbox().left, y - pcell.bbox().bottom
                )
