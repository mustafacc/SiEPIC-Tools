...
class Ring_Filter_DB(PCellDeclarationHelper):
    """
    ...
    def produce_impl(self):
        ...
        t = Trans(
            Trans.R0, (self.r + self.w / 2) / dbu, (self.r + self.g + self.w) / dbu
        )
        pcell = ly.create_cell(
            "Ring", "GSiP", {"layer": LayerSi, "radius": self.r, "width": self.w}
        )
        ...
        # Create the MH Layer
        poly = (
            Path(arc(self.r / dbu, angle_min_MH, angle_max_MH), w_mh)
            .transformed(t)
            .simple_polygon()
        )
        ...
        shapes(LayerPinRecN).insert(
            Path(
                [
                    Point(x0 - (w_Si3 / 2 + taper_length) + pin_length / 2, 0),
                    Point(x0 - (w_Si3 / 2 + taper_length) - pin_length / 2, 0),
                ],
                w,
            )
        ...
        shapes(LayerPinRecN).insert(
            Text("opt1", Trans(Trans.R0, x0 - (w_Si3 / 2 + taper_length), 0))
        ).text_size = (0.5 / dbu)
        ...
        shapes(LayerPinRecN).insert(
            Text("opt2", Trans(Trans.R0, x0 + (w_Si3 / 2 + taper_length), 0))
        ).text_size = (0.5 / dbu)

        shapes(LayerPinRecN).insert(
            Path(
                [
                    Point(x0 - (w_Si3 / 2 + taper_length) + pin_length / 2, y_offset),
                    Point(x0 - (w_Si3 / 2 + taper_length) - pin_length / 2, y_offset),
                ],
                w,
            )
        ...
        shapes(LayerPinRecN).insert(
            Text("opt3", Trans(Trans.R0, x0 - (w_Si3 / 2 + taper_length), y_offset))
        ).text_size = (0.5 / dbu)
        ...
        shapes(LayerPinRecN).insert(
            Text("opt4", Trans(Trans.R0, x0 + (w_Si3 / 2 + taper_length), y_offset))
        ).text_size = (0.5 / dbu)
        ...
        shapes(LayerDevRecN).insert(
            Text("Component=Ring_Filter_DB", Trans(Trans.R0, 0, w * 2))
        ).text_size = (0.3 / dbu)
        shapes(LayerDevRecN).insert(
