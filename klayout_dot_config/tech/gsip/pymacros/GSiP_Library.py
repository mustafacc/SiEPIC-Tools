...
files = [
    f
    for f in os.listdir(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), folder)
    )
...
class GSiP(pya.Library):
    def __init__(self):
        ...
        dir_path = os.path.normpath(
            os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "../gds/building_blocks"
            )
        )
        if verbose:
