...
def run_INTC(verbose=False):
    if verbose:
        ...
        )  # Python Lumerical INTERCONNECT integration handle
    ...
    if verbose:
        print(
            "Checking INTERCONNECT integration handle: %s" % INTC
        )  # Python Lumerical INTERCONNECT integration handle
    ...
    if not INTC:  # Not running, start a new session
        ...
        if verbose:
            print(
                "Started new session. INTERCONNECT integration handle: %s" % INTC
            )  # Python Lumerical INTERCONNECT integration handle
        ...
        try:
            ...
            if verbose:
                print(
                    "Re-Started new session. INTERCONNECT integration handle: %s" % INTC
                )  # Python Lumerical INTERCONNECT integration handle
    try:  # check again
        lumapi.evalScript(INTC, "?'Python integration test.';")
    except:
...
def circuit_simulation_monte_carlo(
    ...
    if not params:
        params = {
            "num_wafers": 1,
            "num_dies": 3,
            "histograms": {"fsr": False, "gain": False, "wavelength": False},
            "waf_var": {
                "width": {
            ...
            "waf_to_waf_var": {
                "width": {"std_dev": 5.0},  # [nm], wafer Sigma RMS for width
                "thickness": {"std_dev": 3.0},  # [nm], wafer Sigma RMS for thickness
            },
        }

    if int(params["num_wafers"]) < 1 or int(params["num_dies"]) < 1:
        print(
            "Insufficient number of dies: The number of die per wafer, and number of wafers, for Monte Carlo simulations need to be 1 or more."
        )
        return
