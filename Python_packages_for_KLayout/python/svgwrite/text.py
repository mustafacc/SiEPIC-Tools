    ...
    **TSpan** element.
    ...
    def __init__(
        ...
        if insert is not None:
            if is_string(insert):
                raise TypeError(
                    "'insert' should be a <tuple> or a <list>  with"
                    " at least two elements."
                )
            if x or y:
                raise ValueError(
                    "Use 'insert' and 'x' or 'y' parameter not" " at the same time!"
                )
            x = [insert[0]]
            y = [insert[1]]
...
class TextPath(BaseElement, XLink, Presentation):
    ...
    def __init__(
        self, path, text, startOffset=None, method="align", spacing="exact", **extra
    ):
        """
